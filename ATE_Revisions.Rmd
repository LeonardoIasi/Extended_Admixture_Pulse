...
Plot_all_together_samples_50_gen_flipped <- function(ggdata,cbPalette_viridis){
  ...
  ggplot(data = ggdata, aes(y=factor(Name),x=as.numeric(as.character(value)),colour=factor(Model))) +
...
Plot_sampling_50_gen_GF_Length_flipped <- function(ggdata,cbPalette_viridis){
  ggplot(ggdata,aes(y=variable,x=as.numeric(as.character(value)),colour=factor(Name)))+
    ...
    scale_color_manual("Simulations",
                       values = c(cbPalette_viridis[6],cbPalette_viridis[6]),
                       labels = c("Simple Pulse","Extended Pulse"),
                       guide = guide_legend(reverse=TRUE))
}
...
Result.Table.path_Nea_tm_1 <-'../Revisions_Fig_4/Result_both_Fit_classic_Lomax_simple/Result_file_SIM_Raw_ALDER-Fit-Fig_1_B_corrected-GF_Model_IV-min_dist_Fit-0.05-ascertainment-0.txt'
Result.Table.path_Nea_tm_2 <-'../Revisions_Fig_4/Result_both_Fit_classic_Lomax_d_2000/Result_file_SIM_Raw_ALDER-Fit-Fig_4_revision-GF_Model_IV-min_dist_Fit-0.05-ascertainmentI-downsampled100-ReccNo_correction.txt'

Plot.data_Nea_tm_1 <- Result.Table.paper.parametrization.fn(Result.Table.path = Result.Table.path_Nea_tm_1,Sampling.time.from.GF.End = 0,name = 'More_Nea')
Plot.data_Nea_tm_2 <- Result.Table.paper.parametrization.fn(Result.Table.path = Result.Table.path_Nea_tm_2,Sampling.time.from.GF.End = 0,name = 'More_Nea')
Plot.data_Nea_tm <- rbind(Plot.data_Nea_tm_1,Plot.data_Nea_tm_2)

Plot.data_Nea_tm$True_mean_GF <- 1500
ggdata_t.Nea_tm_mean_GF_exp <- melt(Plot.data_Nea_tm,measure.vars =  c('tm_sp'),id.vars = c('True_mean_GF','True_GF_length','Name'))
ggdata_t.Nea_tm_mean_GF_lomax <- melt(Plot.data_Nea_tm,measure.vars =  c('mean_GF_EP_tm'),id.vars = c('True_mean_GF','True_GF_length','Name'))
ggdata_l.Nea_tm <- melt(Plot.data_Nea_tm,measure.vars =  c('length_GF'),id.vars = c('True_mean_GF','True_GF_length','Name'))

```



``` {r eval=F,figR3_1, message=FALSE, echo=FALSE,warning=FALSE,fig2.pos="H",fig.width=9,fig.height=10,fig.cap="\\label{fig:figR3_1} Comparison between the Hudson only and hybrid Wright-Fischer/Hudson simulations for parameter inference under the simple and extended pulse model  A) Mean time estimates $t_m$ for different gene flow durations $t_d$ all sampled 50 generations after the gene flow ended. B) Duration estimate $t_d$ of the same scenario"}

# Plot 50 gen from Admixture
ggdata_t.Nea_tm_mean_GF_exp$Model <- "Simple Pulse"
ggdata_t.Nea_tm_mean_GF_lomax$Model <- "Extended Pulse"
...
ggdata_all_together_samples_Nea_tm <- rbind(ggdata_t.Nea_tm_mean_GF_exp,ggdata_t.Nea_tm_mean_GF_lomax)
ggdata_all_together_samples_Nea_tm$Sample_Time <- 0
#ggdata_sampling_50_gen_GF_Length <- ggdata_reodering_fn(ggdata_l.GF_50)
ggdata_sampling_Nea_tm_gen_GF_Length <- ggdata_l.Nea_tm
ggdata_sampling_Nea_tm_gen_GF_Length$Model <- "Extended Pulse"
ggdata_sampling_Nea_tm_gen_GF_Length$Sample_Time <- 0

Mean_GF_Nea_tm <- Plot_all_together_samples_50_gen_flipped(ggdata = ggdata_all_together_samples_Nea_tm,cbPalette_viridis)
Duration_GF_Nea_tm <- Plot_sampling_50_gen_GF_Length_flipped(ggdata = ggdata_sampling_Nea_tm_gen_GF_Length,cbPalette_viridis)
Constant_sampling_time <- ggarrange(Mean_GF_Nea_tm,Duration_GF_Nea_tm,labels = c("A","B"),
...
``` {r eval=F,figR3_3, message=FALSE, echo=FALSE,warning=FALSE,figR3_3.pos="H",fig.width=9,fig.height=10,fig.cap="\\label{fig:figR3_3} Neandertal duration simple and extended pulse AIC and F test comparison"}


AIC_comp=data.frame(AIC_SP=Plot.data_Nea_tm$AIC_SP,AIC_EP=Plot.data_Nea_tm$AIC_EP,
                      GF_duration=Plot.data_Nea_tm$True_GF_length)
...
Fit_all_true_segments <- function(file_pattern_constant,file_pattern_varying,mean_HapMap_rr,AAMap_Map,HapMap_Map){
  ...
  True_segments_constant <- lapply(Path_to_Fragment_Files_True_1, Fit_to_segments,header=F,recomb_rate=1e-8,n_Haploid=100,truncation=F)
  ...
  #no correction
  True_segments_varying_no_correction <- lapply(Path_to_Fragment_Files_True_varying, Fit_to_segments,header=F,recomb_rate=mean_HapMap_rr,n_Haploid=100,truncation=F)
  #AAMap correction
  True_segments_varying_AAMap_correction <- lapply(Path_to_Fragment_Files_True_varying, Fit_to_segments,header=F,Path_to_Recomb_Map=AAMap_Map,n_Haploid=100,truncation=F)
  #HapMap correction
  True_segments_varying_HapMap_correction <- lapply(Path_to_Fragment_Files_True_varying, Fit_to_segments,header=F,Path_to_Recomb_Map=HapMap_Map,n_Haploid=100,truncation=F)
  return(c(True_segments_constant,True_segments_varying_no_correction,True_segments_varying_AAMap_correction,True_segments_varying_HapMap_correction))
}

Fit_all_inferred_segments <- function(file_pattern_constant,file_pattern_varying,mean_HapMap_rr,AAMap_Map,HapMap_Map){
  ...
  Inferred_segments_constant <- lapply(Path_to_Fragment_Files_Inferred_Constant, Fit_to_segments,header=T,recomb_rate=1e-8,n_Haploid=100,truncation=T,lower_trunc=0.05,upper_trunc=1.2)
  ...
  #no correction
  Inferred_segments_varying_no_correction <- lapply(Path_to_Fragment_Files_Inferred_varying, Fit_to_segments,header=F,recomb_rate=mean_HapMap_rr,n_Haploid=100,truncation=F)
  #AAMap correction
  Inferred_segments_varying_AAMap_correction <- lapply(Path_to_Fragment_Files_Inferred_varying, Fit_to_segments,header=F,Path_to_Recomb_Map=AAMap_Map,n_Haploid=100,truncation=F)
  #HapMap correction
  Inferred_segments_varying_HapMap_correction <- lapply(Path_to_Fragment_Files_Inferred_varying, Fit_to_segments,header=F,Path_to_Recomb_Map=HapMap_Map,n_Haploid=100,truncation=F)
  return(c(Inferred_segments_constant,Inferred_segments_varying_no_correction,Inferred_segments_varying_AAMap_correction,Inferred_segments_varying_HapMap_correction))
}

## Variation in recombinations (HapMap) ##
AAMap_Map="/home/leonardo_iasi/Desktop/Neandertal_Human_Introgression_Project/Paper/Paper_Scripts/Simulations/Recombination_Maps/Scaled_Genetic_AAMap.txt"
HapMap_Map="/home/leonardo_iasi/Desktop/Neandertal_Human_Introgression_Project/Paper/Paper_Scripts/Simulations/Recombination_Maps/Scaled_HapMap.txt"
AAMap_Map=read.table(AAMap_Map,stringsAsFactors = F,header=T,fill = T)
HapMap_Map=read.table(HapMap_Map,stringsAsFactors = F,header=T,fill = T)
...
## Fitting True segments

True_seg_td_1 <- Fit_all_true_segments("ALL_Simulation-merged-_Scenario_1_","ALL_Simulation-merged-_Scenario_1_Recomb_",mean_HapMap_rr,AAMap_Map,HapMap_Map)
True_seg_td_200 <- Fit_all_true_segments("ALL_Simulation-merged-_Scenario_2_","ALL_Simulation-merged-_Scenario_2_Recomb_",mean_HapMap_rr,AAMap_Map,HapMap_Map)
True_seg_td_400 <- Fit_all_true_segments("ALL_Simulation-merged-_Scenario_3_","ALL_Simulation-merged-_Scenario_3_Recomb_",mean_HapMap_rr,AAMap_Map,HapMap_Map)
True_seg_td_800 <- Fit_all_true_segments("ALL_Simulation-merged-_Scenario_4_","ALL_Simulation-merged-_Scenario_4_Recomb_",mean_HapMap_rr,AAMap_Map,HapMap_Map)
True_seg_td_1000 <- Fit_all_true_segments("ALL_Simulation-merged-_Scenario_5_","ALL_Simulation-merged-_Scenario_5_Recomb_",mean_HapMap_rr,AAMap_Map,HapMap_Map)
True_seg_td_1500 <- Fit_all_true_segments("ALL_Simulation-merged-_Scenario_6_","ALL_Simulation-merged-_Scenario_6_Recomb_",mean_HapMap_rr,AAMap_Map,HapMap_Map)
True_seg_td_2000 <- Fit_all_true_segments("ALL_Simulation-merged-_Scenario_7_","ALL_Simulation-merged-_Scenario_7_Recomb_",mean_HapMap_rr,AAMap_Map,HapMap_Map)

Inferred_seg_td_1 <- Fit_all_inferred_segments("ALL_Inferred_Archaic_Tracts-_Scenario_1_","ALL_Inferred_Archaic_Tracts-_Scenario_1_Recomb_",mean_HapMap_rr,AAMap_Map,HapMap_Map)
Inferred_seg_td_200 <- Fit_all_inferred_segments("ALL_Inferred_Archaic_Tracts-_Scenario_2_","ALL_Inferred_Archaic_Tracts-_Scenario_2_Recomb_",mean_HapMap_rr,AAMap_Map,HapMap_Map)
Inferred_seg_td_400 <- Fit_all_inferred_segments("ALL_Inferred_Archaic_Tracts-_Scenario_3_","ALL_Inferred_Archaic_Tracts-_Scenario_3_Recomb_",mean_HapMap_rr,AAMap_Map,HapMap_Map)
Inferred_seg_td_800 <- Fit_all_inferred_segments("ALL_Inferred_Archaic_Tracts-_Scenario_4_","ALL_Inferred_Archaic_Tracts-_Scenario_4_Recomb_",mean_HapMap_rr,AAMap_Map,HapMap_Map)
Inferred_seg_td_1000 <- Fit_all_inferred_segments("ALL_Inferred_Archaic_Tracts-_Scenario_5_","ALL_Inferred_Archaic_Tracts-_Scenario_5_Recomb_",mean_HapMap_rr,AAMap_Map,HapMap_Map)
Inferred_seg_td_1500 <- Fit_all_inferred_segments("ALL_Inferred_Archaic_Tracts-_Scenario_6_","ALL_Inferred_Archaic_Tracts-_Scenario_6_Recomb_",mean_HapMap_rr,AAMap_Map,HapMap_Map)
Inferred_seg_td_2000 <- Fit_all_inferred_segments("ALL_Inferred_Archaic_Tracts-_Scenario_7_","ALL_Inferred_Archaic_Tracts-_Scenario_7_Recomb_",mean_HapMap_rr,AAMap_Map,HapMap_Map)
```
...
deCODE_Map="/home/leonardo_iasi/Desktop/Neandertal_Human_Introgression_Project/Paper/Paper_Scripts/Simulations/Recombination_Maps/Scaled_Decode_Map.txt"
AAMap_Map="/home/leonardo_iasi/Desktop/Neandertal_Human_Introgression_Project/Paper/Paper_Scripts/Simulations/Recombination_Maps/Scaled_Genetic_AAMap.txt"
HapMap_Map="/home/leonardo_iasi/Desktop/Neandertal_Human_Introgression_Project/Paper/Paper_Scripts/Simulations/Recombination_Maps/Scaled_HapMap.txt"
deCODE_Map_x=read.table(deCODE_Map,stringsAsFactors = F,header=T,fill = T)
AAMap_Map=read.table(AAMap_Map,stringsAsFactors = F,header=T,fill = T)
HapMap_Map=read.table(HapMap_Map,stringsAsFactors = F,header=T,fill = T)
...
#AAMap correction
True_segments_1_gen_GF_recomb_varying_AAMap_correction <- lapply(Path_to_Fragment_Files_True_3, Fit_to_segments,header=F,Path_to_Recomb_Map=AAMap_Map,n_Haploid=100,truncation=F)
#deCODE correction
True_segments_1_gen_GF_recomb_varying_deCODE_correction <- lapply(Path_to_Fragment_Files_True_3, Fit_to_segments,header=F,Path_to_Recomb_Map=deCODE_Map,n_Haploid=100,truncation=F)
...
```{r eval=F,message=FALSE, echo=FALSE,warning=FALSE}
#load("Segment_fit.RData")
load("Segment_fit_2.RData")
get_seg_result_table <- function(res.list,GF_length,descrip){
  Seg_result.table <- c()
  for(i in 1:10){
...
Plot_tm <- function(ggdata,cbPalette_viridis,main){
  ...
  ggdata$rr <- factor(ggdata$rr,
                          levels = c("varying deCODE correction","varying AAMap correction","varying no correction","constant"),ordered = TRUE)
...
Plot_LRT<- function(Data,Colour_P,main){
  Data$recomb <- factor(Data$recomb,
                      levels = c("constant","varying no correction","varying AAMap correction","varying deCODE correction"),ordered = TRUE)
...
True_segments=rbind(get_seg_result_table(True_segments_1_gen_GF_recomb_constant,1,"constant"),
     get_seg_result_table(True_segments_1_gen_GF_recomb_varying_no_correction,1,"varying no correction"),
     get_seg_result_table(True_segments_1_gen_GF_recomb_varying_AAMap_correction,1,"varying AAMap correction"),
     get_seg_result_table(True_segments_1_gen_GF_recomb_varying_deCODE_correction,1,"varying deCODE correction"),
...
True_segments$td_est <-  4*True_segments$tm_ep*True_segments$k^(-(1/2))
...
``` {r eval=F,figR3_4, message=FALSE, echo=FALSE,warning=FALSE,fig2.pos="H",fig.width=9,fig.height=10,fig.cap="\\label{fig:figR3_4} "}
...
Direct_segments_inf <- ggarrange(True_tm,True_td,Inferred_tm,Inferred_td,labels = c("A","B","C","D"),
          ncol = 2, nrow = 2,widths = c(0.6,0.4),common.legend = T,legend = 'bottom')

Direct_segments_inf
```

``` {r eval=F,figR3_5, message=FALSE, echo=FALSE,warning=FALSE,fig2.pos="H",fig.width=9,fig.height=10,fig.cap="\\label{fig:figR3_4} "}
...
Direct_segments_LRT <- ggarrange(True_LRT,Inferred_LRT,labels = c("A","B"),
          ncol = 2, nrow = 2,widths = c(0.5,0.5),common.legend = T,legend = 'top')
Direct_segments_LRT
```
...
``` {r eval=F,figR4, message=FALSE, echo=FALSE,warning=FALSE,fig2.pos="H",fig.width=9,fig.height=10,fig.cap="\\label{fig:figR4} Comparison between the Hudson only and hybrid Wright-Fischer/Hudson simulations for parameter inference under the simple and extended pulse model  A) Mean time estimates $t_m$ for different gene flow durations $t_d$ all sampled 50 generations after the gene flow ended. B) Duration estimate $t_d$ of the same scenario"}
...
```{r fig3_2,message=FALSE, echo=FALSE,warning=FALSE,fig3.1.pos="H",fig.width=4,fig.height=4,fig.cap="\\label{fig:fig3_2} GLM effect size estimates and 95% C.I. for the parameters: admixture model (simple/extended), recombination rate (constant/varying), demography (simple/complex), minimal genetic distance (0.02/0.05 cM) and ascertainment scheme (LES/HES), on the standardized difference between simulated and estimated admixture time. Estimates are calculated across all possible combinations of parameters and are given as the estimate of the standard model plus the respective parameter estimate. Dotted horizontal line indicates unbiased admixture estimates."}
