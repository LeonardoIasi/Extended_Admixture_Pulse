...
```{r eval=T, message=FALSE, echo=FALSE,warning=FALSE, cache=T}
...
Fig_2_1 <- Read_GLM_indata("/mnt/scratch/leonardoi/Paper_Revisions/Fig_2_A_revision/Result_classic_Exp/")
...
Fig_2_2 <- Read_GLM_indata("/mnt/scratch/leonardoi/Paper_Revisions/Fig_2_B_complex_revision/Result_classic_Exp/")
...
Fig_2_3 <- Read_GLM_indata("/mnt/scratch/leonardoi/Paper_Revisions/Fig_2_C_corrected_revision/Result_classic_Exp/")
...
Fig_2_4 <- Read_GLM_indata("/mnt/scratch/leonardoi/Paper_Revisions/Fig_2_D_Complex_corrected_revision/Result_classic_Exp/")
...
write_csv(x = xdata.M.3,file = "Fig_2_result_tabel.csv")
...
xdata.M.3.no_error <- xdata.M.3.no_error %>% filter(downsampling==100)
xdata.M.3.no_error %>% filter(downsampling==100) %>% filter(AS=="I") %>% filter(Demography=='0_Simple') %>% filter(Recomb.rate=='0_constant') %>% 
  filter(minDist==0.05) %>% summarise(mean(m.exp))
Fig_2_1 %>% filter(downsampling==100) %>% filter(AS=="I") -> xx2
#xdata.M.3 <- xdata.M.3 %>% filter(downsampling!=50)
...
xx3 %>% filter(A==0) %>% filter(MD==0) %>% filter(D==1) %>% filter(R==0) %>% filter(GF==0) %>% summarise(mean(E))

Bdata_index_s_2 <- list(
  E = xdata.M.3.no_error$Diff_s,
  #Id = xdata.M.3$Sim_id_int,
  ...
  #DS = ifelse(xdata.M.3$downsampling==100,0,1)
  
)


Effect_size_fixed_s_2 <- quap(
  alist(
    E ~ dnorm(mu,sigma),
    mu <- a + bA*A + bm*MD + bD*D + bR*R + bG*GF ,
    a ~ dnorm( 0 , 2 ),
    c(bA,bm,bD,bR,bG) ~ dnorm( 0 , 2 ),
    sigma ~ dexp(1)), data=Bdata_index_s_2)


precis(Effect_size_fixed_s_2,depth = 2)
```
