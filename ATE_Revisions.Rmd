...
Plot.data_ALD_red$True_GF_length2 <- factor(Plot.data_ALD_red$True_GF_length,
                    levels = c(1,1000,1500,2000),ordered = TRUE)
#RSS_diff_norm <- Plot.data_ALD_red %>% group_by(Name) %>% group_map(~ (0-(log(.x$RSS_SP)-log(.x$RSS_EP))/sd((log(.x$RSS_SP)-log(.x$RSS_EP))) ) )
#RSS_diff_norm <- Plot.data_ALD_red %>% group_by(Name,True_GF_length2) %>% group_map(~ (0-((.x$RSS_SP)-(.x$RSS_EP))/sd(((.x$RSS_SP)-(.x$RSS_EP))) ))
RSS_diff_norm <- (Plot.data_ALD_red$RSS_EP-Plot.data_ALD_red$RSS_SP)/Plot.data_ALD_red$RSS_SP
RSS_comp=data.frame(
...
P_RSS_comp_results <- ggplot(RSS_comp_results, aes(x=as.factor(Name), y=(RSS_diff_norm),color=as.factor(Name))) + 
                ...
                scale_color_manual("Recombination",
                                   values = c(cbPalette_viridis[12],cbPalette_viridis[1]),
...
P_RSS_comp_supplements <- ggplot(RSS_comp_supplements, aes(x=as.factor(Name), y=(RSS_diff_norm),color=as.factor(Name))) + 
                ...
                scale_color_manual("Recombination",
                                   values = c(cbPalette_viridis[9],cbPalette_viridis[3]),
...
Fig_2_1 <- Read_GLM_indata("/mnt/diversity/leonardo_iasi/Adm_Time_Dating_Sim/Paper_Revisions/Fig_2_A_revision/Result_SP_and_EP/")
Fig_2_1$Demography <- "0_Simple"
Fig_2_1$Recomb.rate <- "0_constant"
Fig_2_1$GF_model <- ifelse(Fig_2_1$Scenario.name=="_Scenario_16_","EP","SP")
...
#Fig_2_2 <- Read_GLM_indata("/mnt/diversity/leonardo_iasi/Adm_Time_Dating_Sim/Paper_Revisions/Fig_2_B_complex_revision/Result_classic_Exp/")
  Fig_2_2 <- Read_GLM_indata("/mnt/diversity/leonardo_iasi/Adm_Time_Dating_Sim/Paper_Revisions/Fig_2_B_complex_Skov_revision/Result_SP_and_EP/")
Fig_2_2$Demography <- "0_Complex"
Fig_2_2$Recomb.rate <- "0_constant"
#Fig_2_2$GF_model <- ifelse(Fig_2_2$Scenario.name=="_Complex_Demographies_Scenario_3_","EP","SP")
Fig_2_2$GF_model <- ifelse(Fig_2_2$Scenario.name=="Skov_Complex_Demographies_Scenario_1_","SP","EP")
...
xdata.M.3_all <- xdata.M.3 %>% 
 filter(downsampling==100 | downsampling==5)

xdata.M.3.no_error <- xdata.M.3.no_error %>% 
 filter(downsampling==100 | downsampling==5)

failed_fits=length(xdata.M.3_all$A.sp)-length(xdata.M.3.no_error$A.sp)
failed_fitspc=failed_fits/length(xdata.M.3_all$A.sp)
...
Effect_size_fixed_stan_2_SP_tm <- ulam(
  alist(
    ...
    sigma ~ dexp(1)), data=Bdata_index_SP,chains = 4,iter = 2000,cores = 4)
'
Effect_size_fixed_stan_3_SP_tm_bias <- ulam(
  alist(
    E ~ dnorm(mu,sigma),
...
Effect_size_fixed_stan_3_SP_tm_deviation <- ulam(
  alist(
    E ~ dnorm(mu,sigma),
    mu <- a + bA*A + bm*MD + bD*D + bR*R + bG*GF + bd*DS + bdA*A*DS + bmA*MD*A + bDR*R*D + bmR*R*MD,
    a ~ dnorm( 0 , 2 ),
...
Effect_size_fixed_stan_2_EP_tm <- ulam(
  alist(
    ...
    sigma ~ dexp(1)), data=Bdata_index_EP_tm,chains = 4,iter = 2000,cores = 4)
'
Effect_size_fixed_stan_3_EP_tm_bias <- ulam(
  alist(
    E ~ dnorm(mu,sigma),
    mu <- a + bA*A + bm*MD + bD*D + bR*R + bG*GF + bd*DS + bdA*A*DS + bmA*MD*A + bDR*R*D + bmR*R*MD,
    a ~ dnorm( 0 , 2 ),
...
Effect_size_fixed_stan_3_EP_tm_deviation <- ulam(
  alist(
    E ~ dnorm(mu,sigma),
    mu <- a + bA*A + bm*MD + bD*D + bR*R + bG*GF + bd*DS + bdA*A*DS + bmA*MD*A + bDR*R*D + bmR*R*MD,
    a ~ dnorm( 0 , 2 ),
...
B_model_result_SP_3 <- precis(Effect_size_fixed_stan_3_SP_tm_bias,prob = 0.95)
B_model_result_SP_3_var <- precis(Effect_size_fixed_stan_3_SP_tm_deviation,prob = 0.95)

B_model_result_EP_3 <- precis(Effect_size_fixed_stan_3_EP_tm_bias,prob = 0.95)
B_model_result_EP_3_var <- precis(Effect_size_fixed_stan_3_EP_tm_deviation,prob = 0.95)
...
Plot_GLM_model_3_effect_sizes <- function(Model,plot_GLM=T){
  ...
  Post_effect_size_est_table <- Post_effect_size_est
  row.names(Post_effect_size_est_table) <- c("Standard Model","Extended Pulse","Varying recombination","Complex Demography","d0 = 0.02","HES","n SNP = 5%",
                                             "Interaction n SNP = 5%/HES","Interaction Complex D./Varying R.", "Interaction d0 = 0.02/HES","Interaction d0 = 0.02/Varying R.")
  Post_effect_size_est_table$Variable <- factor(row.names(Post_effect_size_est_table),
                    levels = c("Standard Model","Extended Pulse","Varying recombination","Complex Demography","d0 = 0.02","HES","n SNP = 5%",
                                             "Interaction n SNP = 5%/HES","Interaction Complex D./Varying R.", "Interaction d0 = 0.02/HES","Interaction d0 = 0.02/Varying R."),ordered = TRUE)
  ...
  P_1 <- ggplot(Post_effect_size_est_table,aes(x=Post_effect_size_est_table$Variable,y=as.numeric(as.character(mean))))+
    ...
    theme(plot.title = element_text(hjust = 0.5,size = 12))+
    #scale_x_discrete(labels= c("Int","G","R","D","d0","A"))+
    scale_x_discrete(labels= c("Standard Model","Extended Pulse","Varying recombination","Complex Demography","d0 = 0.02","HES","n SNP = 5%",
                                             "Interaction n SNP = 5%/HES","Interaction Complex D./Varying R.", "Interaction d0 = 0.02/HES","Interaction d0 = 0.02/Varying R.")
                    ,guide = guide_axis(angle = 90))+
    scale_color_manual("Gene Flow Model",
                       values = c(cbPalette_viridis[6],cbPalette_viridis[1]),
...
SP_GLM_m_3_bias <- Plot_GLM_model_3_effect_sizes(Effect_size_fixed_stan_3_SP_tm_bias,plot_GLM = F)
SP_GLM_m_3_bias$Model <- "SP"
SP_GLM_m_3_bias$GLM <- "bias"
EP_GLM_m_3_bias <- Plot_GLM_model_3_effect_sizes(Effect_size_fixed_stan_3_EP_tm_bias,plot_GLM = F)
EP_GLM_m_3_bias$Model <- "EP"
EP_GLM_m_3_bias$GLM <- "bias"
SP_GLM_m_3_deviation <- Plot_GLM_model_3_effect_sizes(Effect_size_fixed_stan_3_SP_tm_deviation,plot_GLM = F)
SP_GLM_m_3_deviation$Model <- "SP"
SP_GLM_m_3_deviation$GLM <- "Variance"
EP_GLM_m_3_deviation <- Plot_GLM_model_3_effect_sizes(Effect_size_fixed_stan_3_EP_tm_deviation,plot_GLM = F)
EP_GLM_m_3_deviation$Model <- "EP"
EP_GLM_m_3_deviation$GLM <- "Variance"

Model_3_GLM_both <- rbind(SP_GLM_m_3_bias,EP_GLM_m_3_bias,SP_GLM_m_3_deviation,EP_GLM_m_3_deviation)
Model_3_GLM_both$Model <-  factor(Model_3_GLM_both$Model,
                    levels = c("SP","EP"),ordered = TRUE)
Model_3_GLM_both$GLM <-  factor(Model_3_GLM_both$GLM ,
                    levels = c("bias","Variance"),labels = c("bias","abs. deviation"),ordered = TRUE)
ggplot(Model_3_GLM_both,aes(x=Model,y=as.numeric(as.character(mean))))+
        geom_point(aes(size=1.5,col=Model),show.legend = T,size=2)+
...
Result.Table.paper.parametrization.fn <- function(Result.Table.path,Sampling.time.from.GF.End,name){
  ...
  #Result.Table.comparison$length_GF <- (sqrt(Result.Table$mean_GF_lomax_s/(((1/Result.Table$w.lomax))*Result.Table$s.lomax)))*4
  #Result.Table.comparison$length_GF <- 4*Result.Table$tm.ep*Result.Table$k.ep^(-(1/2))
  Result.Table.comparison$length_GF <- 4*Result.Table$tm.ep*(1/Result.Table$k.ep)^(-(1/2))
  Result.Table$length_GF <- Result.Table.comparison$length_GF
...
Plot_all_together_samples_50_gen_flipped <- function(ggdata,cbPalette_viridis){
  ggdata$Model <- factor(ggdata$Model,
                          levels = c("Simple Pulse","Extended Pulse"),ordered = TRUE)
  ggdata$Name <- factor(ggdata$Name,
                        levels = c("Recent_50_HapMap_no_correction","Recent_50_HapMap_AAMap_correction","Recent_50_HapMap_HapMap_correction","Recent_50_constant"),ordered = TRUE)
  ...
  ggplot(data = ggdata, aes(y=factor(Name),x=as.numeric(as.character(value)),colour=factor(Name))) +
    ...
    scale_color_manual("Simulations",
                       values = c(cbPalette_viridis[3],cbPalette_viridis[9],cbPalette_viridis[12],cbPalette_viridis[1]),
                       labels = c("HapMap/Constant","HapMap/AAMap","HapMap/HapMap","Constant/Constant"),
                       guide = guide_legend(reverse=TRUE))
...
Result.Table.path_Recent_50 <-'../Close_to_GF_End_Recent_GF_corrected/Result_with_AIC_F_Test_new_port_corrected_1_over_k/Result_file_SIM_Raw_ALDER-Fit-Close_to_GF_End_Recent_GF_corrected-GF_Model_IV-min_dist_Fit-0.05-ascertainment-0-No_correction.txt'
Result.Table.path_Recent_50_Recomb_Map_no_correction <-'../Close_to_GF_End_Recent_GF_Recomn_Map_Hap_Map_corrected/Result_with_AIC_F_Test_new_port_corrected_1_over_k/Result_file_SIM_Raw_ALDER-Fit-Close_to_GF_End_Recent_GF_Recomn_Map_Hap_Map_corrected-GF_Model_IV-min_dist_Fit-0.05-ascertainment-0-No_correction.txt'
Result.Table.path_Recent_50_Recomb_Map_AAMap_correction <-'../Close_to_GF_End_Recent_GF_Recomn_Map_Hap_Map_corrected/Result_with_AIC_F_Test_new_port_corrected_1_over_k/Result_file_SIM_Raw_ALDER-Fit-Close_to_GF_End_Recent_GF_Recomn_Map_Hap_Map_corrected-GF_Model_IV-min_dist_Fit-0.05-ascertainment-0-AAMap_correction.txt'
Result.Table.path_Recent_50_Recomb_Map_HapMap_Recomb_HapMap_correction <-'../Close_to_GF_End_Recent_GF_Recomn_Map_Hap_Map_corrected/Result_with_AIC_F_Test_new_port_corrected_1_over_k/Result_file_SIM_Raw_ALDER-Fit-Close_to_GF_End_Recent_GF_Recomn_Map_Hap_Map_corrected-GF_Model_IV-min_dist_Fit-0.05-ascertainment-0-HapMap_correction.txt'

Result.Table.path_Recent_Varying <-'../Variyng_Time_of_Recent_sampling_GF_l_fixed_corrected/Result_with_AIC_F_Test_new_port_corrected_1_over_k/Result_file_SIM_Raw_ALDER-Fit-Variyng_Time_of_Recent_sampling_GF_l_fixed_corrected-GF_Model_IV-min_dist_Fit-0.05-ascertainment-0-No_correction.txt'
Result.Table.path_Recent_Varying_Recomb_Map_no_correction <-'../Variyng_Time_of_Recent_sampling_GF_l_fixed_Recomn_Map_Hap_Map_corrected/Result_with_AIC_F_Test_new_port_corrected_1_over_k/Result_file_SIM_Raw_ALDER-Fit-Variyng_Time_of_Recent_sampling_GF_l_fixed_Recomn_Map_Hap_Map_corrected-GF_Model_IV-min_dist_Fit-0.05-ascertainment-0-No_correction.txt'
Result.Table.path_Recent_Varying_Recomb_Map_AAMap_correction <-'../Variyng_Time_of_Recent_sampling_GF_l_fixed_Recomn_Map_Hap_Map_corrected/Result_with_AIC_F_Test_new_port_corrected_1_over_k/Result_file_SIM_Raw_ALDER-Fit-Variyng_Time_of_Recent_sampling_GF_l_fixed_Recomn_Map_Hap_Map_corrected-GF_Model_IV-min_dist_Fit-0.05-ascertainment-0-AAMap_correction.txt'
...
Plot.data_50_complete <- Plot.data_50
...
#RSS_diff_norm <- Plot.data_50_complete %>% group_by(Scenario.name,Name) %>% group_map(~ (0-((.x$RSS_SP)-(.x$RSS_EP))/sd(((.x$RSS_SP)-(.x$RSS_EP))) ) )
#RSS_diff_norm <- Plot.data_50_complete$RSS_EP - Plot.data_50_complete$RSS_SP
RSS_diff_norm <- (Plot.data_50_complete$RSS_EP - Plot.data_50_complete$RSS_SP)/Plot.data_50_complete$RSS_SP
...
RSS_comp_50$Name <- factor(RSS_comp_50$Name,
                    levels = c("Recent_50_constant","Recent_50_HapMap_HapMap_correction","Recent_50_HapMap_AAMap_correction","Recent_50_HapMap_no_correction"),ordered = TRUE)
...
P_RSS_comp_50 <- ggplot(RSS_comp_50, aes(x=as.factor(Name), y=(RSS_diff_norm),color=as.factor(Name))) + 
                ...
                theme(
                  ...
                  scale_color_manual("Recombination",
                                   values = c(cbPalette_viridis[1],cbPalette_viridis[12],cbPalette_viridis[9],cbPalette_viridis[3]),
                                   labels = c("Constant/Constant","HapMap/HapMap","HapMap/AAMap","HapMap/Constant"))

#Plot.data_varying_complete <- Plot.data_varying[complete.cases(Plot.data_varying),]
Plot.data_varying_complete <- Plot.data_varying
...
#RSS_diff_norm <- Plot.data_varying_complete$RSS_EP - Plot.data_varying_complete$RSS_SP
RSS_diff_norm <- (Plot.data_varying_complete$RSS_EP - Plot.data_varying_complete$RSS_SP)/Plot.data_varying_complete$RSS_SP
...
```{r eval=F,table_Supplements_GLM_EP_SP_tm_effect_size_bias_and_var, echo=FALSE,results='asis',message=F,warning=F }
...
```{r table_Supplements_GLM_EP_SP_tm_effect_size_bias, echo=FALSE,results='asis',message=F,warning=F }
print_B_model_SP_result_bias <- Plot_GLM_model_3_effect_sizes(Effect_size_fixed_stan_3_SP_tm_bias,plot_GLM = F)
print_B_model_SP_result_bias <- print_B_model_SP_result_bias[,c(1,2,3)]
print_B_model_SP_result_bias$Model <- "Simple Pulse"
print_B_model_SP_result_bias$Variable <- rownames(print_B_model_SP_result_bias)
print_B_model_SP_result_bias$GLM <- "bias"

print_B_model_EP_result_bias <- Plot_GLM_model_3_effect_sizes(Effect_size_fixed_stan_3_EP_tm_bias,plot_GLM = F)
print_B_model_EP_result_bias <- print_B_model_EP_result_bias[,c(1,2,3)]
print_B_model_EP_result_bias$Model <- "Extended Pulse tm"
print_B_model_EP_result_bias$Variable <- rownames(print_B_model_EP_result_bias)
print_B_model_EP_result_bias$GLM <- "bias"



print_B_model_result_bias <- rbind(print_B_model_SP_result_bias,print_B_model_EP_result_bias)
print_B_model_result_bias <- print_B_model_result_bias[, c(4,6,5,1,2,3)]
kable(print_B_model_result_bias , "latex",row.names=F,col.names = c( "model","GLM","variable","mean" , "5.5%"  ,"94.5%"  ),digits = 2,
      caption = "\\label{tab:tableS1}  Mean, Standard deviation, 5.5/94.5 compatibility interval of the posterior distribution for every parameter effect on the standardized difference between true and estimated admixture time.") %>%
  collapse_rows(columns = 1, latex_hline = "major", valign = "middle")%>%
  kable_styling(latex_options = "HOLD_position")
```

```{r table_Supplements_GLM_EP_SP_tm_effect_size_deviation, echo=FALSE,results='asis',message=F,warning=F }
print_B_model_SP_result_deviation <- Plot_GLM_model_3_effect_sizes(Effect_size_fixed_stan_3_SP_tm_deviation,plot_GLM = F)
print_B_model_SP_result_deviation <- print_B_model_SP_result_deviation[,c(1,2,3)]
print_B_model_SP_result_deviation$Model <- "Simple Pulse"
print_B_model_SP_result_deviation$Variable <- rownames(print_B_model_SP_result_deviation)
print_B_model_SP_result_deviation$GLM <- "abs. deviation"

print_B_model_EP_result_deviation <- Plot_GLM_model_3_effect_sizes(Effect_size_fixed_stan_3_EP_tm_deviation,plot_GLM = F)
print_B_model_EP_result_deviation <- print_B_model_EP_result_deviation[,c(1,2,3)]
print_B_model_EP_result_deviation$Model <- "Extended Pulse tm"
print_B_model_EP_result_deviation$Variable <- rownames(print_B_model_EP_result_deviation)
print_B_model_EP_result_deviation$GLM <-  "abs. deviation"
print_B_model_result_deviation <- rbind(print_B_model_SP_result_deviation,print_B_model_EP_result_deviation)
print_B_model_result_deviation <- print_B_model_result_deviation[, c(4,6,5,1,2,3)]
kable(print_B_model_result_deviation , "latex",row.names=F,col.names = c( "model","GLM","variable","mean" , "5.5%"  ,"94.5%"  ),digits = 2,
      caption = "\\label{tab:tableS1} table_Supplements_EP_tm_td_and_SP_model_3_predictions Mean, Standard deviation, 5.5/94.5 compatibility interval of the posterior distribution for every parameter effect on the standardized difference between true and estimated admixture time.") %>%
  collapse_rows(columns = 1, latex_hline = "major", valign = "middle")%>%
  kable_styling(latex_options = "HOLD_position")
```
...
Extended_Pulse_fixed_k_fn <- function(Data,affine,td,use_log=F){
  ...
  if(use_log==F) {
    ...
    fit1_lomax <- nls(wcorr ~ c+A*(1/(1  + ((tm/ ((1/((td/(4*tm))^2)))) *(dist/100)) ))^((1/(((td/(4*tm))^2)))), start=par1_lomax, 
                          control=list(maxiter=100000, warnOnly=TRUE,minFactor=0.0004))
  }
