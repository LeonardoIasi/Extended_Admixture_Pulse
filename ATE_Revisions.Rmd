...
Result.Table.path_ALD_constant <-'/mnt/diversity/leonardo_iasi/Adm_Time_Dating_Sim/Paper_Revisions/Segment_ALD_comparison_ALD/Result_with_AIC_F_Test_new_port_corrected_1_over_k/Result_file_SIM_Raw_ALDER-Fit-Segment_ALD_comparison_ALD-GF_Model_IV-min_dist_Fit-0.05-ascertainmentI-downsampled100-ReccNo_correction.txt'

# constant interpolation
Result.Table.path_ALD_no_correction <-'/mnt/diversity/leonardo_iasi/Adm_Time_Dating_Sim/Paper_Revisions/Segment_ALD_comparison_recombination_map_ALD/Result_with_AIC_F_Test_new_port_corrected_1_over_k/Result_file_SIM_Raw_ALDER-Fit-Segment_ALD_comparison_recombination_map_ALD-GF_Model_IV-min_dist_Fit-0.05-ascertainmentI-downsampled100-ReccNo_correction.txt'
Result.Table.path_ALD_AAMap_correction <-'/mnt/diversity/leonardo_iasi/Adm_Time_Dating_Sim/Paper_Revisions/Segment_ALD_comparison_recombination_map_ALD/Result_with_AIC_F_Test_new_port_corrected_1_over_k/Result_file_SIM_Raw_ALDER-Fit-Segment_ALD_comparison_recombination_map_ALD-GF_Model_IV-min_dist_Fit-0.05-ascertainmentI-downsampled100-ReccAAMap_correction.txt'
Result.Table.path_ALD_HapMap_correction <-'/mnt/diversity/leonardo_iasi/Adm_Time_Dating_Sim/Paper_Revisions/Segment_ALD_comparison_recombination_map_ALD/Result_with_AIC_F_Test_new_port_corrected_1_over_k/Result_file_SIM_Raw_ALDER-Fit-Segment_ALD_comparison_recombination_map_ALD-GF_Model_IV-min_dist_Fit-0.05-ascertainmentI-downsampled100-ReccHapMap_correction.txt'
...
Extended_Pulse_fn <- function(Data,affine,use_log=F){
  ...
  if(use_log==F) {
    ...
    fit1_lomax <- nls(wcorr ~ c+A*(1/(1  + ((tm/ (1/k)) *(dist/100)) ))^(1/k), start=par1_lomax, algorithm="port", 
                      lower=c(1/1e10,100,1e-6,-1), upper=c(1/2,5000,1,1),control=list(maxiter=100000, warnOnly=TRUE,minFactor=0.0004))
...
plot_ALD_fit <- function(input,lval=0.05,hval=0.5,tm,td,use_log=F,main,legend=F,margins=c(0.25,0.5,0,0.5)){
  ...
  SP_tail <- function(dist,x) x[2]*exp(-(dist)*x[1])+x[3]
  
  EP_tail <- function(dist,x) x[4] + x[3]* (1/(1 + (  (x[1] / (x[2]))*(dist) )))^(x[2])
 ...
 ' 
  ...
  data_x$Data <- "ALD"
  data_x$td <- td
  Fit_Results <- data.frame(x=data_x$M,y=(EP_tail(data_x$M,c(tm,k,coef(Fit_EP_fixed)[1],coef(Fit_EP_fixed)[2]))),fit="True")
  Fit_Results <- rbind(Fit_Results,data.frame(x=data_x$M,y=(SP_tail(data_x$M,c(coef(Fit_SP)[2],coef(Fit_SP)[1],coef(Fit_SP)[3]))),fit="SP_fit"))
  Fit_Results <- rbind(Fit_Results,data.frame(x=data_x$M,y=(EP_tail(data_x$M,c(coef(Fit_EP)[2],coef(Fit_EP)[1],coef(Fit_EP)[3],coef(Fit_SP)[4]))),fit="EP_fit" ))
  '
  #Fit_Results$fit <- factor(Fit_Results$fit,
  #                levels = c("SP_true","SP_fit","EP_true","EP_fit"),ordered = TRUE)
  Fit_Results$fit <- factor(Fit_Results$fit,
...
P_fit_SEG <- function(Fit_res,Data_res,legend=F){
  ...
  Fit_res$fit <- factor(Fit_res$fit ,
                        levels = c("True","SP_fit","EP_fit"),ordered = TRUE)
  ...
  theme(strip.text.y = element_blank(),
    ...
    coord_cartesian(ylim = c(2,max(Data_res$dens)),
                    xlim=c(0.0005,0.003))+
  scale_linetype_manual("Fit",
                        values=c( "solid", "dashed","twodash"),
                        labels = c(c("True","fitted SP","fitted EP")))+
...
P_fit_ALD <- function(Fit_res,Data_res,legend=T){
  ...
  #geom_line(data=Fit_res,aes(x = x, y = y),show.legend = legend)+
  facet_grid(vars(as.factor(RR)),vars(as.factor(Data2)))+
  theme(axis.text.x = element_text(size=10, angle=90),
        strip.text.x = element_text(size = 7))+
  coord_cartesian(ylim = c(-4,max(Data_res$ALD)),
                    xlim=c(0.0005,0.003))+
  scale_linetype_manual("Fit",
                        values=c( "solid", "dashed","twodash"),
                        labels = c(c("True","fitted SP","fitted EP")))+
...
```{r dpi = 350,eval=T,figResult2_all_together, message=FALSE, echo=FALSE,warning=FALSE,fig2.pos="H",fig.width=9,fig.height=12,fig.cap="\\label{fig:figResult2_all_together} "}
...
save(Effect_size_fixed_stan_3_SP_tm_bias,file = "Effect_size_fixed_stan_3_SP_tm_bias.RData")
...
save(Effect_size_fixed_stan_3_SP_tm_deviation,file = "Effect_size_fixed_stan_3_SP_tm_deviation.RData")
...
save(Effect_size_fixed_stan_3_EP_tm_bias,file = "Effect_size_fixed_stan_3_EP_tm_bias.RData")
...
save(Effect_size_fixed_stan_3_EP_tm_deviation,file = "Effect_size_fixed_stan_3_EP_tm_deviation.RData")
...
```{r eval=T, message=FALSE, echo=FALSE,warning=FALSE, cache=T, results='hide'}
load("Effect_size_fixed_stan_3_SP_tm_bias.RData")
load("Effect_size_fixed_stan_3_SP_tm_deviation.RData")
load("Effect_size_fixed_stan_3_EP_tm_bias.RData")
load("Effect_size_fixed_stan_3_EP_tm_deviation.RData")
B_model_result_SP_3_bias <- precis(Effect_size_fixed_stan_3_SP_tm_bias,prob = 0.95)
B_model_result_SP_3_deviation <- precis(Effect_size_fixed_stan_3_SP_tm_deviation,prob = 0.95)

B_model_result_EP_3_bias <- precis(Effect_size_fixed_stan_3_EP_tm_bias,prob = 0.95)
B_model_result_EP_3_deviation <- precis(Effect_size_fixed_stan_3_EP_tm_deviation,prob = 0.95)

Plot_GLM_model_3_effect_sizes <- function(Model,sigma=F){
  ...
  if(sigma==F){
    ...
    row.names(Post_effect_size_est_table) <- c("Standard Model","Extended Pulse","Varying\nRecombination","Complex\nDemography","d0 = 0.02","HES","n SNP = 5%",
                                             "Interaction\nn SNP = 5%/HES","Interaction\nComplex D./Varying R.", "Interaction\nd0 = 0.02/HES","Interaction\nd0 = 0.02/Varying R.")
    Post_effect_size_est_table$Variable <- factor(row.names(Post_effect_size_est_table),
                      levels = c("Standard Model","Extended Pulse","Varying\nRecombination","Complex\nDemography","d0 = 0.02","HES","n SNP = 5%",
                                             "Interaction\nn SNP = 5%/HES","Interaction\nComplex D./Varying R.", "Interaction\nd0 = 0.02/HES","Interaction\nd0 = 0.02/Varying R."),ordered = TRUE)
      ...
      Post_effect_size_est=data.frame(mean=c(
        ...
        mean(post_effect_size_model$bmA),
        mean(post_effect_size_model$bmR),
        mean(post_effect_size_model$sigma)),
        HPDI_lower=c(
        HPDI(post_effect_size_model$a,0.95)[1],
        HPDI(post_effect_size_model$bG,0.95)[1],
        HPDI(post_effect_size_model$bR,0.95)[1],
        ...
        HPDI(post_effect_size_model$bmA,0.95)[1],
        HPDI(post_effect_size_model$bmR,0.95)[1],
        HPDI(post_effect_size_model$sigma,0.95)[1]),
        HPDI_upper=c(
        HPDI(post_effect_size_model$a,0.95)[2],
        HPDI(post_effect_size_model$bG,0.95)[2],
        HPDI(post_effect_size_model$bR,0.95)[2],
        ...
        HPDI(post_effect_size_model$bmA,0.95)[2],
        HPDI(post_effect_size_model$bmR,0.95)[2],
        HPDI(post_effect_size_model$sigma,0.95)[2])
        )
        Post_effect_size_est_table <- Post_effect_size_est
  row.names(Post_effect_size_est_table) <- c("Standard Model","Extended Pulse","Varying Recombination","Complex Demography","d0 = 0.02","HES","n SNP = 5%",
                                             "Interaction n SNP = 5%/HES","Interaction Complex D./Varying R.", "Interaction d0 = 0.02/HES","Interaction d0 = 0.02/Varying R.", "Sigma")
  Post_effect_size_est_table$Variable <- factor(row.names(Post_effect_size_est_table),
                    levels = c("Standard Model","Extended Pulse","Varying Recombination","Complex Demography","d0 = 0.02","HES","n SNP = 5%",
                                             "Interaction n SNP = 5%/HES","Interaction Complex D./Varying R.", "Interaction d0 = 0.02/HES","Interaction d0 = 0.02/Varying R.", "Sigma"),ordered = TRUE)
      return(Post_effect_size_est_table)
  }
...
SP_GLM_m_3_bias <- Plot_GLM_model_3_effect_sizes(Effect_size_fixed_stan_3_SP_tm_bias,sigma = F)
SP_GLM_m_3_bias$Model <- "SP"
SP_GLM_m_3_bias$GLM <- "bias"
EP_GLM_m_3_bias <- Plot_GLM_model_3_effect_sizes(Effect_size_fixed_stan_3_EP_tm_bias,sigma = F)
EP_GLM_m_3_bias$Model <- "EP"
EP_GLM_m_3_bias$GLM <- "bias"
SP_GLM_m_3_deviation <- Plot_GLM_model_3_effect_sizes(Effect_size_fixed_stan_3_SP_tm_deviation,sigma = F)
SP_GLM_m_3_deviation$Model <- "SP"
SP_GLM_m_3_deviation$GLM <- "Variance"
EP_GLM_m_3_deviation <- Plot_GLM_model_3_effect_sizes(Effect_size_fixed_stan_3_EP_tm_deviation,sigma = F)
...
```{r dpi = 350,figResult_3_GLM_SP_and_EP_bias,message=FALSE, echo=FALSE,warning=FALSE,fig3.1.pos="H",fig.width=6,fig.height=4,fig.cap="\\label{fig:figResult_3_GLM_SP_and_EP_model_3_var} SP and EP var GLM effect size estimates and 95% C.I. for the parameters: admixture model (simple/extended), recombination rate (constant/varying), demography (simple/complex), minimal genetic distance (0.02/0.05 cM) and ascertainment scheme (LES/HES), on the standardized difference between simulated and estimated admixture time. Estimates are calculated across all possible combinations of parameters and are given as the estimate of the standard model plus the respective parameter estimate. Dotted horizontal line indicates unbiased admixture estimates."}
...
Model_3_GLM_both <- rbind(SP_GLM_m_3_bias,EP_GLM_m_3_bias)
Model_3_GLM_both$Model <-  factor(Model_3_GLM_both$Model,
                    levels = c("SP","EP"),ordered = TRUE)
ggplot(Model_3_GLM_both,aes(x=Model,y=as.numeric(as.character(mean))))+
        geom_point(aes(size=1.5,col=Model),show.legend = T,size=2)+
...
```{r figResult_3_GLM_SP_and_EP_deviation,message=FALSE, echo=FALSE,warning=FALSE,fig3.1.pos="H",fig.width=6,fig.height=4,fig.cap="\\label{fig:figResult_3_GLM_SP_and_EP_model_3_var} SP and EP var GLM effect size estimates and 95% C.I. for the parameters: admixture model (simple/extended), recombination rate (constant/varying), demography (simple/complex), minimal genetic distance (0.02/0.05 cM) and ascertainment scheme (LES/HES), on the standardized difference between simulated and estimated admixture time. Estimates are calculated across all possible combinations of parameters and are given as the estimate of the standard model plus the respective parameter estimate. Dotted horizontal line indicates unbiased admixture estimates."}
...
Model_3_GLM_both <- rbind(SP_GLM_m_3_deviation,EP_GLM_m_3_deviation)
Model_3_GLM_both$Model <-  factor(Model_3_GLM_both$Model,
                    levels = c("SP","EP"),ordered = TRUE)
Model_3_GLM_both$GLM <-  factor(Model_3_GLM_both$GLM ,
                    levels = c("Variance"),labels = c("abs. deviation"),ordered = TRUE)
ggplot(Model_3_GLM_both,aes(x=Model,y=as.numeric(as.character(mean))))+
        geom_point(aes(size=1.5,col=Model),show.legend = T,size=2)+
        facet_grid(GLM~Variable,switch = "x",scales = "free")+
        geom_errorbar(aes(ymin=HPDI_lower,ymax=HPDI_upper,pos=as.numeric(Variable)),col="black",width=0.2)+
...
print_B_model_SP_result_bias <- Plot_GLM_model_3_effect_sizes(Effect_size_fixed_stan_3_SP_tm_bias,sigma = T)
...
print_B_model_SP_result_bias$n_eff <- round(B_model_result_SP_3_bias$n_eff,0)
print_B_model_SP_result_bias$Rhat <- round(B_model_result_SP_3_bias$Rhat4,1)

print_B_model_EP_result_bias <- Plot_GLM_model_3_effect_sizes(Effect_size_fixed_stan_3_EP_tm_bias,sigma = T)
...
print_B_model_EP_result_bias$n_eff <- round(B_model_result_EP_3_bias$n_eff,0)
print_B_model_EP_result_bias$Rhat <- round(B_model_result_EP_3_bias$Rhat4,1)


print_B_model_result_bias <- rbind(print_B_model_SP_result_bias,print_B_model_EP_result_bias)
print_B_model_result_bias <- print_B_model_result_bias[, c(4,6,5,1,2,3,7,8)]
kable(print_B_model_result_bias , "latex",row.names=F,col.names = c( "model","GLM","variable","mean" , "5.5%"  ,"94.5%"  ,"n eff", "Rhat"  ),digits = 2,
...
print_B_model_SP_result_deviation <- Plot_GLM_model_3_effect_sizes(Effect_size_fixed_stan_3_SP_tm_deviation,sigma = T)
...
print_B_model_SP_result_deviation$n_eff <- round(B_model_result_SP_3_deviation$n_eff,0)
print_B_model_SP_result_deviation$Rhat <- round(B_model_result_SP_3_deviation$Rhat4,1)

print_B_model_EP_result_deviation <- Plot_GLM_model_3_effect_sizes(Effect_size_fixed_stan_3_EP_tm_deviation,sigma = T)
...
print_B_model_EP_result_deviation$n_eff <- round(B_model_result_EP_3_deviation$n_eff,0)
print_B_model_EP_result_deviation$Rhat <- round(B_model_result_EP_3_deviation$Rhat4,1)

print_B_model_result_deviation <- rbind(print_B_model_SP_result_deviation,print_B_model_EP_result_deviation)


print_B_model_result_deviation <- print_B_model_result_deviation[, c(4,6,5,1,2,3,7,8)]
kable(print_B_model_result_deviation , "latex",row.names=F,col.names = c( "model","GLM","variable","mean" , "5.5%"  ,"94.5%" ,"n eff", "Rhat" ),digits = 2,
...
P_RSS_comp_50 <- ggplot(RSS_comp_50, aes(x=as.factor(Name), y=(RSS_diff_norm),color=as.factor(Name))) + 
                geom_boxplot(show.legend = T)+
                facet_grid( Data~as.factor(True_td_named)+as.factor(True_tm_named),switch = "x")+
                geom_hline( aes(yintercept = 0 ))+
                labs(y = "Normalized\n RSS Difference ")+
                labs(x = "")+
...
```{r dpi = 350,fig5_Real_data, message=FALSE, echo=FALSE,warning=FALSE,fig.width=12,fig.height=8,fig.cap="\\label{fig:fig5_Real_data} ."}
...
Fit_stats <- data.frame(model=c("Simple Pulse","Extended Pulse (td=1)","Extended Pulse (td=100)",
                                        "Extended Pulse (td=200)","Extended Pulse (td=400)","Extended Pulse (td=800)",
                                        "Extended Pulse (td=1000)","Extended Pulse (td=1500)","Extended Pulse (td=2000)",
                                        "Extended Pulse (td=2500)"),RSS=Model_RSS)

kable(Fit_stats , "latex",row.names = F,col.names = c("Model"  , "RSS"),
