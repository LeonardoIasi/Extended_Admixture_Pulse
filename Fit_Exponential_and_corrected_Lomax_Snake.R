...
### the lomax used is shape= k+1 and scale= k/tm
### for the fitting we define shape = w =1/k and scale= s = k/tm / k = 1/tm
...
Fit_Lomax_fn <- function(Data,affine,Expo_s,Fix_lambda){
  ...
  if(Fix_lambda){
    ...
    if(affine){
      ...
      fit1_lomax <- nls(wcorr ~ c+A*(1/(1  + (((w)*(dist/100)) / Expo_s)))^((1/w)+1), start=par1_lomax, control=list(maxiter=10000, warnOnly=TRUE,minFactor=0.0004))
      
    } else {
      fm1_lomax <- function(x)  x[3]* (1/(1 + ((x[1])*(dist/100) /  x[2])))^((1/x[1])+1)
      fm2_lomax_k <- function(x) sum((wcorr-fm1_lomax(x))^2)
      ...
      fit1_lomax <- nls(wcorr ~ A*(1/(1  + (((w)*(dist/100)) / Expo_s)))^((1/w)+1), start=par1_lomax, control=list(maxiter=10000, warnOnly=TRUE,minFactor=0.0004))
    ...
    if(affine){
      ...
      fit1_lomax <- nls(wcorr ~ c+A*(1/(1  + ((w*(dist/100)) / s)))^((1/w)+1), start=par1_lomax, control=list(maxiter=10000, warnOnly=TRUE,minFactor=0.0004))
      ...
      fit1_lomax <- nls(wcorr ~ A*(1/(1  + (((w)*(dist/100)) / s)))^((1/w)+1), start=par1_lomax, control=list(maxiter=10000, warnOnly=TRUE,minFactor=0.0004))
