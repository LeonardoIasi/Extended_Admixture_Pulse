...
### uses classic lomax with shape = alpha (k) and scale = lambda (tm/k)
...
Only_Simple_Pulse_fit <- as.logical(snakemake@params[['Only_EXPO_fit']])
...
Fit_Lomax_fn <- function(Data,affine,Expo_s,Fix_lambda){
  ...
  dist=xx$dist
  wcorr=xx$wcorr
  # here lambda = w and alpha = s
  print("Estimating classic lomax parameter")
  if(affine){
    ...
    fit1_lomax <- nls(wcorr ~ c+A*(1 + (dist/100)/ w )^(-s), start=par1_lomax, control=list(maxiter=10000, warnOnly=TRUE,minFactor=0.0004))
    ...
    fit1_lomax <- nls(wcorr ~  A*(1 + (dist/100)/ w )^(-s), start=par1_lomax, control=list(maxiter=10000, warnOnly=TRUE,minFactor=0.0004))
