...
disequilibrium (ALD) decay curve (Eq. \ref{eq:1}), with  $\lambda = -td$ as
...
distance for each SNP. We used the other map to assign distances by linear interpolation
...
realistic and complex demographic history with substructure in the ancestral human population and additional gene flow between Africans and non-Africans after the Neandertal admixture (Figure \ref{fig:Intro_fig} B) . The 
effective population
sizes are based on estimated from Neandertal and present-day human
...
same as in the simple simulations (2550 generations ago). The population size of the ancestor of Neandertals and
...
We compared estimates for different mean admixture times ranging from 250 generations ago to 2000 generations ago. We either used a pulse-like gene flow model where the gene flow is only one generation long or a continuous gene flow model for the simulations. The duration of gene flow under the continuous model is 50\% of the mean time (if the mean time is 250 generations ago gene flow ranges from 325 to 175 generations ago). Assuming only a one generation pulse to estimated the mean time, revealed only minor deviations between the two scenarios and the true
...
In conclusion, the differences in mean time estimates between a pulse and a continuous admixture assuming a one-generation pulse are moderate making point estimates for mean admixture time compatible with long continuous admixture.
...
estimated around 20 \% younger in previous studies(\cite{sankararaman_date_2012}). We used the African-American genetic map of chromosome 1 for our simulations and only its mean recombination rate to assign a genetic length between a pair of SNPs, to check the effect of uncertainties in the variation of recombination events. We compared this admixture time estimates under the varying recombination map to the former used constant one.
...
admixture (\cite{vernot_resurrecting_2014,hsieh_whole-genome_2016}) by
generating segments rich in divergent variantas in high LD to each
other. Figure \ref{fig:Intro_fig} B shows the two demographies of
the Neandertal admixture used throughout this study. The simplified one without population size
changes was used till now. To check for the effect of demography we used a more complex model including substructure in the ancestral human
population, where human populations in Africa are subdivided with a low
...
distribution. Besides this, admixture segments can also
get lost by genetic drift, resulting in less data and noisier fitting.
Moreover, an additional admixture event as shown in Figure
\ref{fig:Intro_fig} B after the Neandertal admixture event between the
African and admixed non-African population was shown to potentially
