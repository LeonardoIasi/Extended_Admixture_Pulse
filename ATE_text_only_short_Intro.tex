...
enabling to obtain a duration of continuous admixture. We used both admixture models on real data. We fitted several admixture scenarios with different durations to ALD data from the 1000 Genomes Project with 3 high coverage Neandertals as well as to directly inferred Neandertal fragments using data from Skov et al. 2020 \ref{}.
...
ancestry in Africans. Each simulation was repeated 100 times. We used a simplified version of the demographic model described in Skov et al. 2018 for the directly inference of admixture segments. Here we only simulate Eurasians with a starting effective population size of 3899. Number of chromosomes, length of the chromosome, mutation rate, recombination rate and all split times are the same as in the ALD simulations. We sampled 500 haploid Africans and 100 haploid Eurasians.
...
of time since the admixture event $t$ with $l$ ,here being either the genetic
...
\mathbb{E}[x] &= \frac{1}{\lambda} = \frac{1}{k\theta}
\end{split}
\end{equation}
...
equal. Here \(x\) is in generations. $t_{m}$ is the mean time of admixture equal to the mean of the Gamma distribution ($\mathbb{E}[m]$) and $Var[m]=(\frac{t_d}{4})^2$ its variance with $t_d$ as the duration of admixture in generations.
...
\begin{itemize}
    ...
    \item The segment length distribution $L_i$ follows a Lomax-distribution, i.e. has the analytical density $Pr(L=l) = \frac{1}{\theta k-1} (1+l\lambda\frac{1}{\theta})^{-(k-1)}$
