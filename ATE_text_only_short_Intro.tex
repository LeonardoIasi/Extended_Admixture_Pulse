...
Through the sequencing of Neandertal genomes we ahev learned that modern humans both encountered , and admixed with Neandertals during their expansion into Eurasia. These interactions resulted in Neandertal DNA making up 2-3\% of the genomes of all non-Afican individuals \citep{green_draft_2010}. The timing and duration of the admixture events that produce this signal, however remains a question \cite{sankararaman_date_2012,moorjani_genetic_2016}.

Here, we introduce an extended admixture pulse model that allows joint estimation of the timing and duration of gene flow. This model contains two parameters, one for the time of gene flow, and one for the duration of gene flow whilst retaining much of the mathematical simplicity of the simple pulse model. 
...
The first step in dating admixture events from genetic data is therefore estimating the length distribution of admixture segments.  There are two main approaches for this; a first set of methods starts with identifying all admixture segments over a certain length, and then uses the length distribution of these segments for inference. Alternatively, the length distribution can also be estimated from patterns of linkage along a chromosome directly, without explicitly inferring the genomic location of these segments \citep{chimusa_dating_2018} (Figure \ref{fig:fig1} B).
 ...
 In the first set of approaches, the identification of segments is largely independent from the later dating, and can be done using a variety of methods \citep{seguin_orlando_paleogenomics_2014,vernot_excavating_2016,sankararaman_combined_2016,racimo_signatures_2017,skov_detecting_2018}. These approaches are most useful for recent admixture not older than 200 generations ago, estimated on high-quality data, as uncertainty in the segment identification is not easily fed forward into the timing inference \citep{hellenthal_genetic_2014}.
...
The evidence for more complex patterns of Neandertal gene flow is more spotty, as most Neandertal admixture segments have a similar divergence to the sequenced Neandertals \citep{browning_analysis_2018}. However, an example of  Neandertal gene flow private to a local population is the identification of admixture segments spanning several Mbp in \textit{Oase 1}, the genome of an early modern human from Romania. He had a recent Neandertal ancestor less than 200 years before he lived (~37–42 kya), later than the age of the \textit{Ust'Ishim} modern human which is already admixed with Neadertals. However, the population \textit{Oase 1}  belonged to did  not contribute substantially to present-day human populations  \citep{fu_genome_2014,fu_early_2015}.
...
\cite{ralph_geography_2013} used the distribution of shared identical by descent (IBD)  segments length between pairs of individuals to infer the number and age of genetic common ancestors. By modeling over all ancestors and ages they fit more complex migration patterns. However, a major problem they found is that a large set of migration patterns fit the IBD distribution equally well. To coup with this they introduced a regularization scheme to their likelihood function by adding a penalization term \citep{ralph_geography_2013}.
...
The extended pulse model assumes that the migration rate over time is Gamma distributed, so that the  distribution of admixture segment lengths has a closed form (Figure \ref{fig:fig1} C \& D) with mean admixture time and duration as the two parameters.
...
The dotted line indicates the oldest possible time of gene flow. D) The expected LD decay under the extended pulse model.}
\end{figure}
...
between simple and extended pulse gene flow for different admixture times.
