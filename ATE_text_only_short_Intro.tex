...
Given $T_i$, we assume that $L_i$ is exponentially distributed with rate parameter given the admixture time $t_a$ and the recombination rate $r$.
\begin{equation}
\label{eq:generall_length_distribution}
    P(L_i=l|T_i=t) \sim \text{Exp}[t_a r]
\end{equation}

If we measure the length of each segment $L_i$ in the recombination distance Morgan (M), $r=1$.
...
In the simple pulse model, we assume that all fragments enter the population at the same time $t_a$. Therefore all $T_i$ have the same value $t_a$, and $T$ thus is a constant distribution. We can realize this by using a Dirac delta function, which is zero everywhere except at $t_a$ and integrates to 1 over the entirety of the distribution:
...
Here, we define the admixture duration $t_d=4\sqrt{\frac{t_m^2}{k}}$, which is a convenient measure for the duration of gene flow. The distribution of segment length is then given by:
...
The distribution  in equation \ref{eq:Likelihood_function_extended_pulse} is known as a \emph{Lomax} or \emph{Pareto-II} distribution, which is a heavier-tailed relative of the Exponential distribution. It is typically parameterized using a shape parameter $\alpha=k$ and scale parameter $\lambda = \frac{t_m}{k}$. 

\begin{equation}
\label{eq:Classic_Lomax}
    p(x) = \frac{\alpha}{\lambda} \Bigg( \frac{k}{1+\frac{x}{\lambda}}\Bigg)^{-(\alpha+1)}
\end{equation}
...
Var[L] = \frac{(\frac{t_m}{k})^2k}{(k-1)(k-2)}
\end{equation}
...
P(L_i > d) = \left( \frac{1}{1 + \frac{k}{tm} \:d}\right) ^k
\end{equation}

For inference, following \cite{moorjani_genetic_2016}, we add an intercept $A$ and a constant modelling background LD $c$. to the tail functions. The genetic distance $d$ is measured in centMogran (cM).
