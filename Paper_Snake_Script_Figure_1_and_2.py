...
def simulation(params,Folder_name,master_name,number, master,GF_Model):
	...
	samples=[
		...
		msprime.Sample(population=2, time=int(params['End_of_Neandertals'])),
		]	

	population_configuration=[
		msprime.PopulationConfiguration(sample_size=int(params['sample_Africans']),
			initial_size=int(params['N_Africans'])),	
		msprime.PopulationConfiguration(sample_size=int(params['sample_non_Africans']),
			initial_size=int(params['N_non_Africans'])),
		msprime.PopulationConfiguration(sample_size=int(params['sample_Neandertals']),
			initial_size=int(params['N_Neandertals']))
		]


	if str(master['GF_Model'][0]) == 'GF_Model_IV':
		Neandertal_Gene_Flow_absolute_start=msprime.MigrationRateChange(time=int(params['End_of_Neandertals']),rate=0)
