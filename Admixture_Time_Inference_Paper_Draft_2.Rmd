...
Violations of these assumptions are known to influence the mean time estimates. Hence, the effect of assuming a one-generation pulse for a multi-generation long admixture has to be contratsed with other potential biases. Some of the most known are listed below.
<!-- last sentence changed to be more positive by Ben's recommendationc -->
...
when admixture is very old $(t \approx 2N)$ or very recent $t \approx \log(2N)$ or when the admixture proportions are large [@liang_lengths_2014]. 
<!-- In our case we deal with small admixture proportions and $\log(2N) < t < 2N$. -->
...
Different genetic maps are highly correlated on a megabase scale but reveal significant population differences on a finer scale [@kong_fine-scale_2010; @hinch_landscape_2011]. The average length of introgressed segments when dating ancient admixture events is on o kb scale [@sankararaman_date_2012;@vernot_resurrecting_2014], and so fine-scale recombination maps are essential. 
...
known. 
This is relevant because while demography does not alter the recombination clock significantly, it can create false signals of admixture [@vernot_resurrecting_2014; @hsieh_model_based_2016], by generating segments rich in divergent variantas in high LD to each other. Figure \ref{fig:Intro_fig} B gives an example of potential demographic biases for the neandertal-human admixture by comparing two possible demographic scenarios. One is a simplified model without population size changes. The other is a more complex one 
...
segments [@nei_linkage_1973;@pfaff_population_2001]. 
...
Here, only positions in the admixed population are considered where both source populations are diverged from each other. A problem arises, when introgressed variants from the ancestral 
population are ascertained using only a few sequenced individuals as 
proxies for the actual introgressing population, which is often the case in dating archaic admixture. [@sankararaman_date_2012;@fu_genome_2014;@sankararaman_combined_2016;@moorjani_genetic_2016]. These individuals are potentially only distantly related to the introgressing population [@jacobs_multiple_2019], which might 
result in missing introgressed variants. 


##Dates of archaic introgression obtained so far

All these assumptions play a critical role in admixture date estimates for ancient admixture time.  



Both approaches, the ALD and the direct inference of haplotypes, have been used to estimate the time of admixture between Neandertals and modern humans assuming a one generation admixture pulse.  Using the decay of introgressed neandertal variants in modern day genomes, Sankararaman et al dated the neandertal-human admixture time to be 37,000–86,000 ya (years ago) [@sankararaman_date_2012]. Later, this data was refined to 40,510–54,454 ya (95% CI) using a different ascertainment scheme combined with a different genetic map [@moorjani_genetic_2016]. A date was also obtained from an ancient genome to be 50,000 - 60,000 ya by adding the admixture date of the ancient individual, obtained by the decay of pairwise covariance between introgressed SNPs, to the specimens radiocarbon date [@fu_genome_2014]. 
...
Using simulations, the effect of each assumption is obtained and explicitly modeled conditioning on all other assumptions to get individual effect sizes. In the next step we want to explicitly model continuous admixture. Knowledge about a strong biasing effects will help to model it, which is mathematically more challenging than a pulse admixture and the fitting process is more prone to interference by biasing effects.
