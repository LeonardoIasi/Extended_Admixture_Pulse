...
### Demography
Third, the demography is known. Figure \ref{fig:Into_fig} B shows two possible demographies for the Neandertal admixture, a simplified one without no population changes and a more complex one inclding structured populations, bottlenecks and additional gene flow. These demographies can have an influence on admixture time estimates since demographic events can create false signals of archaic introgression. Various processes such as extrem reduction in population sizes of the admixing population can create non-admixture LD [@schaper_linkage_2012]. Substructure in an ancestral human population, where human population in Africa are subdivided with a low migration rate, as shown in Figure \ref{fig:Into_fig} B, can also introduce divergent chromosomal segment with variants in strong LD, mimicing Archaic introgressed segments [@nei_linkage_1973]; @pfaff_population_2001]. Moreover additional gene flow after the Neandertal admixture event between the African and admixed non-African population was shown to potentialy influence the detected amount of Neandertal ancestry [@petr_limits_2019] as well as the time estimates [@sankararaman_date_2012]. 
...
### Pulse model
Finally, the admixture is assumed to happened in only a single generation. This single pulse is mathematically convinient since all the introgressed segments entering in this single generation can be modeled using a single exponential distribution. This however limits the information gained form the dating since in case of a multi generation long admixture the single-pulse model results in a mean admixture time between start end end of the gene flow. Idealy one would like to obtaine the start and end, hence the duration of admixture between Neandertals and non-African populations. This could hold implications about the contact between Neandertals and modern humans, e.g. was gene flow a one isolated incident or common over the whole time of their co-existance. Neandertals and modern human populations overlapped in some regions for a long period of time [@higham_timing_2014], potentially resulting in continuous admixture over hundreds of generations.
If admixture is happening over $n$ generations this results in a mixture of $n$ different exponential distribution one for each set of introgressed segments per generation [@pickrell_ancient_2014].  Previous studies tried to model this, using the ALD, by assuming not a continuous admixture but summaryzing it into different independent events. In this case, the decay curve is comprised out of $n$ different exponential terms, each for one admixture event [@pickrell_ancient_2014; @zhou_inference_2017]. Methods were introduced to model continuous admixture using a polynomial function with the number of coefficients related to the number of generations of continuous admixture [@zhou_modeling_2017]. These models however work best with recent admixture, in the last 100 generations, with a substential admixture proportion of around 0.3 [@zhou_modeling_2017]. This is outsode of the range for scenarios of Neandertal admixture, were the admixture proportion is around 0.03 and the timing around 1500 generations ago [@sankararaman_date_2012;@prufer_complete_2013;@sankararaman_genomic_2014].
