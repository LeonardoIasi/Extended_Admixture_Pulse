...
Third, the demography is known. Figure \ref{fig:Into_fig} shows two possible demographies for the Neandertal admixture. These demographies can have an influence on admixture time estimates since demographic events can create false signals of archaic introgression. Various processes such as extrem reduction in population sizes of the admixing population can create non-admixture LD [@schaper_linkage_2012]. Substructure in an ancestral human population, where human population in Africa are subdivided with a low migration rate, as shown in Figure xx, can also introduce divergent chromosomal segment with variants in strong LD, mimicing Archaic introgressed segments [@nei_linkage_1973] [@pfaff_population_2001]. Moreover additional gene flow after the Neandertal admixture event between the African and admixed non-African population was shown to potentialy influence the detected amount of Neandertal ancestry [@petr_limits_2019] as well as the time estimates [@sankararaman_date_2012]. 
<!-- also in [@sankararaman_combined_2016] ??? -->
<!-- Not sure about the last sentece -->
...
Finally, the admixture is assumed to happened in only a single generation. This assumption of pulse like events might be violated, since e.g. in the case of archaic and modern human admixture, populations overlapped in some regions  for a long period of time [@higham_timing_2014], potentially resulting in continuous admixture over hundreds of generations.
Expand:
...
<!-- ![Introduction Figure](/home/leonardo_iasi/Desktop/ATE_Paper/Admixture_Time_Inference_Paper/Paper_Graphics.pdf) -->

``````{r Into_fig,message=FALSE, echo=FALSE,warning=FALSE,fig.align="center",fig.pos="H",fig.cap="\\label{fig:Into_fig} A) Direct and ALD approach B) Demographic scanarios"}
library(png)
