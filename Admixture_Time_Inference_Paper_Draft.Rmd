...
#Introduction
\textcolor{green}{*bp, li 28/02: in generall motivate why you are talking about stuff, why is it important for our question and dont go into explenaition straight away*}
...
In addition admixture is recognized to shapes genetic diversity and can introduce novel variants into the admixing populations [@harrison_hybridization_2014]. These variants can be adaptive in the admixed population [@hedrick_adaptive_2013] or be selected against at loci which determine species specific phenotypes [@shaw_genes_2011], thereby helping to identify loci important for local adaptation [@payseur_using_2010]. Admixture is found in many sexually reproducing species of plants, invertebrates and vertebrates [@payseur_genomic_2016]. Novel DNA sequencing methods make it easier to detect admixture event between populations [@sousa_understanding_2013]. The sequencing of the Neandertal [@green_draft_2010] and Denisovan [@reich_genetic_2010] genome revealed admixture events between Neandertals and  modern humans outside of Africa  [@green_draft_2010; @prufer_complete_2013; @vernot_resurrecting_2014;@fu_early_2015; @fu_genome_2014; @sankararaman_genomic_2014;@prufer_high-coverage_2017], as well as a second admixture event between Denisovans and Asian populations [@reich_genetic_2010; @meyer_high-coverage_2012; @sankararaman_combined_2016; @vernot_excavating_2016]. These admixture events where found to have impacted modern human phenotypes [@dannemann_something_2018]. One important question is how long ago these admixture events are and how long they lasted for, since this can be a clue to the debated question of the time of extinction of these hominin populations.


\textcolor{green}{*bp 19/02:this feels very thin. It's OK to motivate admixture as something that is widely present, but then you need to give a comprehensive introduction, not just a very incomplete list of papers. The alternative is to simply refer to a bunch of relevant reviews*}

\textcolor{green}{*li 27/02:I refere now to some hybridization reviews which point out to alot of admixture papers in different species*
...
in this one generation can be modeled using a single exponential distribution. This however limits the information gained from the dating since in case of a 
...
\textcolor{green}{*bp 19/02: Maybe introduce here the term of admixture pulse*}


\textcolor{green}{*bp,li 28/02: Hervorheben der main idea das wir die laenge vom Nean gf messen wollen especially das ende die hauptfage hierbei waere inwieweit das end von einem gf mit dem bisher gemessenen mittelwert entfernt ist. Da wir wissen das der mittelwert durch die ganzen untengennanten faktoren beeinflusst werden kann muessen wir diese auch fuer die laengen messung in betracht ziehen*}
...
Since admixture segments are not directly observable, the challenge is to correctly infer the length of the introgressed segments. There are two main approaches to identify segments for dating admixture  [@chimusa_dating_nodate] (Figure \ref{fig:Intro_fig} A). The first approach uses the admixture-induced linkage disequilibrium (ALD) decay. 
...
\textcolor{green}{*li 27/02: I think it makes sense to mention all the assumptions which might be problematic and in the end how we gonne do it (check the effec of continuous admixture, compare it to the other assumptions, try to explicitly model continuous admixture using the Lomax)*}

##Assumptions on the data
All dates obtained have to be based on a priori knowledge and assumptions.
Both approaches to identify the introgressed segments, the direct measure of the segment length and the indirect ALD based method, without first directly inferring introgressed segments such as the popular ALDER method [@loh_inferring_2013], rely on some fairly strong modelling assumptions [@pool_inference_2009; @gravel_population_2012; @liang_lengths_2014]. Additional to the one-generation pulse simplification, the model assumes that the proportion of admixture of individuals in the admixed population is identical, tracts are rare and inbreeding is not significant, such that admixture segments do not recombine with each other to form longer segments [@pool_inference_2009]. The effect of drift is assumed to be negligible [@loh_inferring_2013], the introgressed segments act neutral [@shchur_distribution_2019] and the recombination map is identical across populations and does not change over time [@gravel_population_2012].
...
\textcolor{green}{*li 27/02: from liang et al 2014: Tract length can be correlated due to the variance in migrant ancestors an individual has and due to inbreeding joining together migrant tracts which violates the iid assumption especially in the cases mentioned* }
...
\textcolor{green}{*bp 19/02: in this paragraph, you repeated yourself a lot. Try to be concise and present your argument in one logical sequence whenever possible* }
...
\textcolor{green}{*bp 19/02:recap what Priya/Sriram found about the bias/correction*}

\textcolor{green}{*li 27/02: now I first explain  how it affects the estimate according to Siriam (Prya quotes him on this) and then what is the problem with the correction and then what is the problem with genetic maps are*}


### Demography
\textcolor{green}{*bp 19/02: this section (and frequently your writing in general) could need more structure. One simple approach is the   context-content-conclusion system For this intro section, the basic idea is that for each paragraph you need to make clear that you start by*}

\textcolor{green}{*1. explaining why we're interested in demography,* }
\textcolor{green}{*2. what precisely the issue is*}
\textcolor{green}{*3. explain what you're gonna do about it*}


A third assumption is that the demographic history of the studied population is
known. While the recombination clock itself is relatively robust to population size 
changes 
\textcolor{green}{*bp 19/02: ref Gravel2012? li 27/02: No they didn't check for fluctuations in Ne*}
, the problem is the detection of truly introgressed segments. Demographic processes can create similar genomic signals as admixture [@vernot_resurrecting_2014; @hsieh_model_based_2016]. These segments also exhibit high density of divergent variants in high LD to each other. There are multiple demographic processes that could lead to such signals. Figure \ref{fig:Intro_fig} B shows two possible demographies for the neandertal admixture, a simplified one without population size changes and a more complex one 
...
Here, only positions in the admixed population are considered where both source populations are diverged from each other. The problem here is that introgressed variants from the ancestral archaic 
population are ascertained using only a few sequenced archaic individuals as 
proxies for the actual introgressing archaic population [@sankararaman_date_2012;@fu_genome_2014;@sankararaman_combined_2016;@moorjani_genetic_2016]. These individuals are potentially only distantly related to the introgressing population [@jacobs_multiple_2019], which might 
result in missing introgressed variants. 

\textcolor{green}{*li 27/02 What to cite for the last sentence?*}
...
\textcolor{green}{*bp 19/02: preceeding section could be merged with last intro paragraph*}

\textcolor{green}{*bp 19/02: this should be at the end of the intro*}

\textcolor{green}{*li 27/02: Now it is at the end and i merged it with the existing paragraph as well as the previous attempts of modeling multi-generation long admixture*}
