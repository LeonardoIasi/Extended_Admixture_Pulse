...
Detecting admixture, i.e. gene flow between previously isolated populations can shed light into the complex evolutionary history of populations. The sequencing of the Neandertal [@green_draft_2010] and Denisovan [@reich_genetic_2010] genome revealed admixture events between Neandertals and  modern humans outside of Africa  [@green_draft_2010; @prufer_complete_2013; @fu_early_2015; @fu_genome_2014; @sankararaman_genomic_2014], as well as an independent admixture event between Denisovans and Asian populations[@reich_genetic_2010; @meyer_high-coverage_2012; @sankararaman_combined_2016; @vernot_excavating_2016]. 


##Admixture on the genomic level and basic idea how to date it
On the genomic level, admixture introduces highly divergent chromosomal segments into the admixing population. Over time recombination between parental chromosomes progressively break the introgressed segments apart, reducing their length in each generation [@falush_inference_2003] (Figure \ref{fig:Into_fig} A). Under a constant recombination rate, the length of introgressed segments should be inversely proportional to time since the admixture event. Assuming the recombination events occurring according to a Poisson process, the fragment length distribution can be approximated by an exponential distribution, holding the number of generations since the fragments entered the admixing population [@pool_inference_2009]. Hence, the length distribution of introgressed chromosomal segments can be used to inferred the time since the admixture event, also known as the recombination clock [@pool_inference_2009; @gravel_population_2012]. To do this, the model to estimate the time of the admixture event usually assumes an one generation long admixture event followed by a period of isolation [@liang_lengths_2014]. The proportion of admixture of individuals in the admixed population is assumed to be identical, tracts are rare and inbreeding is not significant, such that admixture segments do not recombin with echother to form longer segments [@pool_inference_2009].  The effect of drift is assumed to be negligible [@loh_inferring_2013], the introgressed segments act neutral [@shchur_distribution_2019] and the recombination rate does not change over time [@gravel_population_2012].
...
The dating of admixture events is a question of great interest. The challenge here is to correctly infer the length of the introgressed segments. There are two main approaches to date admixture [@chimusa_dating_nodate]. The first approach uses the admixture-induced linkage disequilibrium (ALD) decay. 
...
As recombination breaks the introgressed chromosomal segments apart each generation, the association between variants on these introgressed segments decays as genetic distance increases. In case of a recent admixture event, ALD is found over long genetic distances [@patterson_methods_2004] and is therefore easy distinguishable from short range non-ALD [@moorjani_history_2011]. For ancient admixture however, a pairwise LD between introgressed variants is calculated using an ascertainment scheme to filter for the introgressed variants in the admixed population, since LD form admixture and LD from other processes is in a similar range length [@sankararaman_date_2012]. The pairwise LD as a function of genetic distant is then fitted using an exponential distribution holding a point estimate for the time since the admixture event [@moorjani_history_2011; @loh_inferring_2013]. 


##The two approaches of obtaining the length of introgressed segments: searching for introgressd haplotypes
The other approach first tries to infer introgressed haplotypes directly, by partitioning an individuals genome into admixture segments [@gravel_population_2012; @price_sensitive_2009; @lawson_inference_2012]. Multiple methods are available focusing on different characteristics introgressed haplotypes, such as LD patterns and density of derived variants, to infer them [@lawson_inference_2012; @racimo_signatures_2017; @seguin_orlando_paleogenomics_2014; @vernot_excavating_2016; @sankararaman_combined_2016; @skov_detecting_2018]. In the second step, the length distribution of these directly inferred introgressed haplotypes can be used for dating.

##Dates of archaic introgression obtained so far
The two approach were used to estimate the Neandertal human admixture to be 37,000–86,000 yBP [@sankararaman_date_2012] as well as 40,510–54,454 yBP (95% CI) [@moorjani_genetic_2016] using modern day genomes and 50,000 to 60,000 yBP using a ancient Genome[@fu_genome_2014]. An additional contemporaneous  admixture event between Neandertals and East Asian population was suggested to explain the higher amount of Neandertal ancestry in these populations [@kim_selection_2015; @vernot_complex_2015]. The Denisovan human admixture was dated to be between 44,000–54,000 yBP [@sankararaman_combined_2016]. Using Papuan individuals revealed two time separated admixture events one in line with previous estimates at 45.7 kya (95% CI 31.9-60.7 kya) and one exclusive to Papuans dated to be around 29.8 kya (95% CI 14.4-50.4 kya) [@jacobs_multiple_2019]. An additional Denisovan admixture event was suggested for East-Asian populations by identifying Denisovan segments, which are more diverged from the Denisovan high-coverage genome then previously found segments [@browning_analysis_2018]. Comparing the mean length of the two groups of segments introgressed from divergent Denisovan populations did, however, not reveal significant differences, suggesting a lack of power to distinguish the two events by time [@browning_analysis_2018; @jacobs_multiple_2019].

##Assumptions on the data
Both approaches, the direct measure of the segment length and the indirect ALD based method, without first directly inferring introgressed segments such as the popular ALDER program [@loh_inferring_2013], typically require some fairly strong assumptions [@pool_inference_2009; @gravel_population_2012; @liang_lengths_2014], which might bias the estiamtes when violated.

### Segments are is independent and identically exponentially distributed
First, it is assumed that the length of the introgressed segments is independent and identically exponentially distributed. This means that the number of ancestors per admixed individual is identical, such that the proportion of introgressed segments is equal among the individuals. This is not true for Neandertal admixture in non-African population [@vernot_complex_2015]. Segments are independent such that segments of the same ancestor do not recombine to form longer segments. Liang and Nielsen 2014 showed that the  violation of the independent and identically exponentially distributed segments length assumption can downward bias admixture time estimates when the true admixture time is very  recent, the number of ancestors is small or the admixing population is small. This increases the chance of admixture segments to recombine into a larger segment mimicking a segment length distribution of a more recent admixture[@liang_lengths_2014].
<!-- Disentengel model and data assumptions -->
<!-- Pool and Nielsen assumption: no significant inbreeding tracts are rare (no recombination with eachother) -->
<!-- Gravel 2012: tracts from multiple migrant ancestors -->

### Recombination Map
Second, it is assumed that the recombination rate is known. Since the recombination is used as a clock the correct genetic distance between introgressed segments or variants on the segments has to be known. Recombination maps are used to assign the genetic length between introgressed segments/variants. Different maps are available. Either focusing on short term information by using parent-offspring data to directly observe recombination events [@coop_high-resolution_2008; @kong_fine-scale_2010] or searching for admixture tracts breakpoints [@hinch_landscape_2011; @wegmann_recombination_2011] or long term population averages of recombination inferred by LD [@mcvean_fine-scale_2004; @myers_fine-scale_2005; @hapmap_consortium_2007]. 
The genetic maps are highly correlated on a megabase scale but reveal significant population differences on a finer scale [@kong_fine-scale_2010; @hinch_landscape_2011]. Furthermore, recombination maps can also vary over time [@auton_fine-scale_2012]. These uncertainties in the genetic map were found to downward bias the time estimates by missing certain recombination events resulting in assuming introgressed segments longer than they actually are [@sankararaman_date_2012].  Sankararaman et al. 2012 suggested a method for accounting for uncertainty in the recombination map by estimating a correction parameter for a given recombination map by comparing the distances between a pair of markers in the map to the number of crossovers that span those markers as observed in a pedigree map. This however limits the applicability of the method, since it requires a population-specific map as well as pedigree based map to estimate the error in the map [@sankararaman_date_2012; @fu_genome_2014; @sankararaman_combined_2016].

### Demography
Third, the demography is known. Figure \ref{fig:Into_fig} B shows two possible demographies for the Neandertal admixture, a simplified one without no population changes and a more complex one including structured populations, bottlenecks and additional gene flow. These demographies can have an influence on admixture time estimates since demographic events can create false signals of archaic introgression. Various processes such as extreme reduction in population sizes of the admixing population can create non-admixture LD [@schaper_linkage_2012]. Substructure in an ancestral human population, where human population in Africa are subdivided with a low migration rate, as shown in Figure \ref{fig:Into_fig} B, can also introduce divergent chromosomal segment with variants in strong LD, mimicking Archaic introgressed segments [@nei_linkage_1973]; @pfaff_population_2001]. Moreover additional gene flow after the Neandertal admixture event between the African and admixed non-African population was shown to potentially influence the detected amount of Neandertal ancestry [@petr_limits_2019] as well as the time estimates [@sankararaman_date_2012]. 
<!-- Not sure about the last sentence -->
<!-- Explain demography using the figure -->

### Ascertainment
An additional bias might be introduced in the ALD based dating method since it requires ascertaining the SNPs for admixture informative sites, especially for archaic admixture [@sankararaman_date_2012; @fu_genome_2014]. The problem here is that introgressed variants from the ancestral archaic population are ascertained using only a few sequenced archaic individuals as proxies for the actual introgressing archaic population. These individuals are potentially only distantly related to the introgressing population, which might result in missing introgressed variants. 
<!-- What to cite for the last sentence?? -->

### Pulse model
Finally, the admixture is assumed to happened in only a single generation. This single pulse is mathematically convenient since all the introgressed segments entering in this single generation can be modeled using a single exponential distribution. This however limits the information gained form the dating since in case of a multi generation long admixture the single-pulse model results in a mean admixture time between start end end of the gene flow. Ideally one would like to obtain the start and end, hence the duration of admixture between Neandertals and non-African populations. This could hold implications about the contact between Neandertals and modern humans, e.g. was gene flow a one isolated incident or common over the whole time of their co-existence. Neandertals and modern human populations overlapped in some regions for a long period of time [@higham_timing_2014], potentially resulting in continuous admixture over hundreds of generations.
If admixture is happening over $n$ generations this results in a mixture of $n$ different exponential distribution one for each set of introgressed segments per generation [@pickrell_ancient_2014].  Previous studies tried to model this, using the ALD, by assuming not a continuous admixture but summarizing it into different independent events. In this case, the decay curve is comprised out of $n$ different exponential terms, each for one admixture event [@pickrell_ancient_2014; @zhou_inference_2017]. Methods were introduced to model continuous admixture using a polynomial function with the number of coefficients related to the number of generations of continuous admixture [@zhou_modeling_2017]. These models however work best with recent admixture, in the last 100 generations, with a substantial admixture proportion of around 0.3 [@zhou_modeling_2017]. This is outside of the range for scenarios of Neandertal admixture, were the admixture proportion is around 0.03 and the timing around 1500 generations ago [@sankararaman_date_2012;@prufer_complete_2013;@sankararaman_genomic_2014].

<!-- This assumption of pulse like events might be violated, since e.g. in the case of archaic and modern human admixture, populations overlapped in some regions  for a long period of time [@higham_timing_2014], potentially resulting in continuous admixture over hundreds of generations. -->

<!-- Expand: -->
<!-- why is single pulse used (mathematically easy) -->
<!-- what do we want to know (start and end of the admixture) -->
<!-- -tested on simulations with no changes in Ne, Admixture proportion of 30% and uniform recombnation [@zhou_modeling_2017]-->
<!-- are there such models -->

Here we aim to evaluate admixture time estimates, whereby we focus on a well studied example of archaic admixture into modern humans. Those admixture events are assumed to be long time in the past, with few, potentially distantly related genomes available from the archaics as proxies for the true introgressing population. We are particularly interested in scenarios of long continuous gene flow between archaic and human populations and its signals in the ALD distribution. We introduce a model for continuous admixture, where the migration rate over time is gamma distributed with two parameter, one for the mean time of continuous admixture and one for the duration of the admixture. First, we examining the effect of long continuous admixture on the admixture time estimates in comparison to the effects of the aforementioned model assumptions. Therefore, we quantitatively evaluate the effect of the admixture model, recombination, demography and analysis parameters on the inference. Second, we define the expectation of the resulting segment length distribution for continuous Gama distributed admixture being Lomax distributed, holding a parameter for the duration of admixture. This expectation works for both methods to infer the segments length, either directly or by using the ALD decay. Using this model, we investigate under which scenarios the parameters of the Lomax-distribution can be  accurately  estimated and for which parameters we can distinguish a pulse-like admixture event from a continuous event. Together this critically evaluates the current state of knowledge about different aspects of archaic admixture time.


``````{r Into_fig,message=FALSE, echo=FALSE,warning=FALSE,fig.align="center",out.width="95%",fig.pos="H",fig.cap="\\label{fig:Into_fig} A) Direct and ALD approach B) Demographic scanarios"}
library(png)
#Intro_Fig <- readPNG('Paper_Graphics.png',native = T,info = T)
include_graphics(path = 'Paper_Graphics.png',auto_pdf = T)
...
Simulations were carried out using the msprime coalescent simulator [@kelleher_efficient_2016] with sample sizes chosen to reflect presently available data: We simulate 176  diploid African individuals and  170 diploid non-Africans, corresponding to the number of haploid Yoruba (YRI) and Central Europeans from Utah (CEU) sequences in the 1000 Genomes project phase 3 data [@the_1000_genomes_project_consortium_global_2015]. Since 3 high coverage Neandertal sequences are available [@prufer_complete_2013][@prufer_high-coverage_2017] we choose to simulate  3 diploid genomes. For each individual we simulated 20 chromosomes with a length of 150 Mb each.The mutation rate was set for all simulations to 2x10-8 per base per generation. The recombination rate was set to 1x10-8  per base pair per generation unless specified otherwise. The demographic parameters are based on previous studies [@sankararaman_date_2012][@fu_genome_2014][@moorjani_genetic_2016]. In the “simple” model, the  effective population size is assumed constant at Ne=10000 for all populations, the split time between modern humans and Neandertals 10000 generations ago and a split time between Africans and non-Africans of 2550 generations ago. The migration rate from Neandertals into non-Africans was set to zero before the split from Africans, to ensure no Neandertal ancestry in Africans.


###Gene Flow

<!-- Maybe one should harmonize the equations with Ben's -->
In the simple model, gene flow is a one generation pulse resulting in an exponentially distributed admixture-induced linkage disequilibrium (ALD) decay curve (Eq. \ref{eq:1}), with $\lambda = -dt$ as the rate parameter of the exponential distribution holding the inverse of time since the admixture event $t$ as a function of genetic distance $d$, 

\begin{equation}
\label{eq:1}
pulse \: ALD \sim exp(\lambda) 
\end{equation}

The migration rate for continuous admixture was modeled as a gamma distribution  (Eq. \ref{eq:2})

\begin{equation}
\label{eq:2}
migration \: rate \sim \Gamma(k+1,\frac{1}{\theta})
\end{equation}

Where k is the shape and theta the rate parameter. The parameter values are chosen such that the mean length $\frac{1}{\lambda}$ of the exponentially distributed ALD decay curve resulting from the one generation admixture pulse, is equal to the mean length of a ALD decay curve, as a result of continuous migration with the same total amount of migrants, modeled using a Lomax distribution (Eq. \ref{eq:3})

\begin{equation}
\label{eq:3}
continuous \: ALD \sim Lomax(k,\theta)
\end{equation}
...
\begin{equation*}
\nonumber
where \qquad k=E[x] \, \theta \qquad and \qquad \theta=\frac{E[x]}{Var[x]}
\end{equation*}

Equation (Eq. \ref{eq:4}) shows the relationships between the distribution parameters such that the resulting decay mean length are equal. Here E[x] is mean admixture time in generations and Var[x] is one fourth of the length of admixture time in generations squared, with x as the number of generations of admixture between two populations.

###Recombination map

To investigate the effect of a more realistic recombination frequency we simulated using a recombination map. We either used the African-American-Map [@hinch_landscape_2011] or the HapMap phase 3 [@noauthor_second_2007] for simulations under a variable recombination rate, for simplicity, we used the same recombination map (150 Mb of chromosome 1, excluding the first 10 Mb) for all simulated chromosomes.  The mean recombination rate was calculated from the 150 Mb map ($1.843*10^{-8} \frac{M}{bp}$ AAMap and $1.549*10^{-8} \frac{M}{bp}$ HapMap) and we used linear interpolation to calculate the genetic position from the physical position between SNPs. For correcting the genetic distance we interpolated the SNPs genetic distance by the map used for correction.

###Inferred demography

To test the impact of demographic history on admixture time estimates, we simulate a more realistic and complex demographic history using estimated effective population sizes and split times. MSMC estimates from YRI as representatives for Africans and CEU for non-Africans from Schiffles & Durbin 2014 [@schiffels_inferring_2014] were used together with PSMC [@li_inference_2011] inferred models from the Vindija33.19 Neandertal. In these models, a mutation rate of 2x10-8 bp/gen and  starting effective population size of 1000 is assumed.  The estimates were corrected for branch shortening of 1000 generation .
To integrate the effective population size at a given time point for modern humans from Schiffels & Durbin 2014 (Figure 4 Excel Table) we first transformed the time points given in years back to generations by using 30 years for one generation, as assumed in the original study. Second, since the original estimates are based on a different mutation rates ($1.25*10^{-8} \frac{bp}{gen}$), we corrected all estimates for the mutation rate used in the simulations ($2*10^{-8} \frac{bp}{gen}$). The split times between Neandertals and modern humans as well as between Africans and non-Africans were kept the same as in the simple simulations (1000  and 2550 generations ago, respectively). To simulate branch shortening caused by the extinction of Neandertals, Neandertals were sampled 750 generations before the Africans and non-Africans
...
The pairwise weighted LD between the ascertained SNPs was calculated using ALDER [@loh_inferring_2013]. A minimal genetic distance $d_0$ between SNPs is set either to 0.02 cM and 0.05 cM. To obtain the mean time estimates the data is fitted an exponential distribution shown in equation \ref{eq:5}, using a non-linear least-square optimization algorithm implemented in R [@R_Core_Team_2019]. Where *A* is the intercept, $\frac{1}{s} = t$ the time since the admixture event in generations, *d* the genetic distance in cM and *c* is a constant modeling background LD. The model was fitted following Moorjani et al 2016 [@moorjani_genetic_2016]. The duration of continuous admixture is modeled using the Lomax fit shown in Eq. \ref{eq:6}. We used the particularization from Kozubowski et al. 2008 [@Kozubowski_2008_TestingEV], here $k = \frac{1}{w}$. The starting value of $s$ is taken from the exponential fit to ensure convergence of the model. Model validity diagnostics like distribution of residuals, residuals plotted against fitted values and model stability diagnostics like dfbetas were checked for selected simulations and showed no obvious deviations from the model assumptions [@quinn_experimental_2002]. To compare the two nested models we used Akaike's information criterion (AIC) measuring the goodness of fit while penalizing the addition of new parameters $p$ and thus controlling for under- and overfitting (Eq. \ref{eq:7}).

\begin{equation}
\label{eq:5}
ALD \sim\ A\,e^{-\frac{d}{s}}+c
\end{equation}

\begin{equation}
\label{eq:6}
ALD \sim\ A\,\left( \frac{1}{1 + \frac{wd}{s}}\right) ^\frac{1}{w}+c
\end{equation}
...
<!-- But how short is that? Which models can we distinguish.  -->
...
Figure_All_Real_Data <- function(input,log,GF_length,Colour_P){
  ...
  Px <-  ggplot(data =Real_Data.melted, aes(x = Genetic_Distance, y = value,color=variable)) +
    geom_point(pch=18)+
    #coord_cartesian(ylim = c(-8,-13),expand = 0,xlim = c(0,0.5)+
...
Theoratical_Lomax <- function(Timespan_GF_Models,max_Genetic_distance,Split_time,Mean_Time,Colour_P){
  n_GF_Models=length(Timespan_GF_Models)
  
  Lomax_fun <- function(GF_Length,max_Genetic_distance,Split_time,Mean_Time,log=T){
    ...
    Theta=b
    k=a
    Lomax_normal <- function(dist,k,theta) (1 + ((dist/100) /  theta))^-(k)
    Lomax_log <- function(dist,k,theta) -k* log(1 + ((dist/100) /  theta))
    if(log==T){
      ALD <-  Lomax_log(dist=Genetic_length,k = k ,theta = Theta)
    }
    else{
      ALD <-  Lomax_normal(dist=Genetic_length,k = k ,theta = Theta)
    }
  ...
  for(i in Timespan_GF_Models){
    xx=Lomax_fun(i,max_Genetic_distance,Split_time,Mean_Time,log=T)
...
Plot_Fig_1_B <- function(Data,Colour_P){
  Px <-  ggplot(Data,aes(x=Var6,y=as.numeric(as.character(Var1)),colour=factor(Var6)))+
    geom_point(aes(size = Freq),show.legend = F)+
    geom_boxplot()+
...
P_Lomax <-Theoratical_Lomax(c(1,200,400,800,1000,1500),0.5,2550,1500,Colour_P = cbPalette_viridis)
...
P1_1 <- Plot_Fig_1_A(Get_Data_Table(Figure_1_A_Data),Colour_P = cbPalette_viridis)

P1_2 <- Plot_Fig_1_B(Get_Data_Table(Figure_1_B_Data),Colour_P = cbPalette_viridis)
...
res.M.3=lm(Diff~Ascertainment+min_dist+Demography+Demography+Recomb.rate+GF,data = xdata.M.3)
...
PX3 <- Plot_Fig_2_X(Simplest_case,xx[xx$Var2=="0_Pulse_0_LES_0.02_Inferred_0_constant",],
             m.coefs["(Intercept)",]-m.coefs["DemographyInferred",],m.coefs["DemographyInferred",],x.lable = c("Simple","Inferred"),"Demography\n")
