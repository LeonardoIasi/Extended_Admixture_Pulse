...
To do this, the model usually assumes a one generation long admixture event followed by a period of isolation [@liang_lengths_2014]. The proportion of admixture of individuals in the admixed population is assumed to be identical, tracts are rare and inbreeding is not significant, such that admixture segments do not recombine with each other to form longer segments [@pool_inference_2009].  The effect of drift is assumed to be negligible [@loh_inferring_2013], the introgressed segments act neutral [@shchur_distribution_2019] and the recombination rate does not change over time [@gravel_population_2012].
...
Both approaches have been used to estimate the time of admixture between Neandertals and modern humans.  Using XXX, Sankararaman et al dated the neandertal-human admixture time to be 37,000–86,000 ya (years ago) [@sankararaman_date_2012]. Later, this data was refined to 40,510–54,454 ya (95% CI) [@moorjani_genetic_2016] *explain why these dates are better estimated*. using modern day genomes and 50,000 to 60,000 ya using an ancient genome [@fu_genome_2014]. An additional contemporaneous *contemporaneous to what?* admixture event between Neandertals and East-Asian populations was suggested to explain the higher amount of Neandertal ancestry in present-day East Asians [@kim_selection_2015; @vernot_complex_2015]. The Denisovan human admixture was dated to be between 44,000–54,000 ya using modern day genomes [@sankararaman_combined_2016] *using what technique?*. An additional Denisovan admixture event was suggested for East-Asian populations by identifying Denisovan segments, which are more diverged from the Denisovan high-coverage genome then previously found segments [@browning_analysis_2018]. Comparing the mean length of the two groups of segments introgressed from divergent Denisovan populations did, however, not reveal significant differences, suggesting a lack of power to distinguish the two events by time [@browning_analysis_2018; @jacobs_multiple_2019]. Analysing genomes from Papuan individuals revealed two time separated admixture events with Denisovan, one in line with previous estimates at 45.7 kya (95% CI 31.9-60.7 kya) and one exclusive to Papuans dated to be around 29.8 kya (95% CI 14.4-50.4 kya) [@jacobs_multiple_2019].
...
and identically exponentially distributed. This means that the number of ancestors
per admixed individual is identical, such that the proportion of 
introgressed segments is equal among the individuals*Is this true? Reference?*. 
One restriction this puts on the data is that segments of the same ancestor are 
assumed to recombine into longer segments. This assumption is violated in particular 
when admixture is very old $(t \approx 2N)$ or very recent $t \approx \log(2N)$ (Liang and Nielsen 2014). 
...
biased *recap what Priya/Sriram
found about the bias/correction*. 

Recombination maps are used to assign the genetic length between introgressed segments/variants. Different maps are available[add refs]. 
...
admixture [@sankararaman_date_2012; @fu_genome_2014]. *explain briefly why this
is necessary, and how exactly this is done*
