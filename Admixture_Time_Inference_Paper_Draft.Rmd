...
Intro_graphic <- readPNG(source = "Paper_Graphics_detedtion_methods.png",native = F,info = T)
im_A <- ggplot() + 
...
Result_graphic <- readPNG(source = "Paper_Graphics_Demography.png",native = F,info = T)
im_B <- ggplot() + 
...
```{r message=FALSE, echo=FALSE,warning=FALSE}
input <- "Real_Data_Analysis/Raw_ALDER_output_ALL_chr_hcNea_YRI_CEU_1k_G_LES_ascertained_corrected_rr_CEU_specific_map.txt"
...
```{r fig5, message=FALSE, echo=FALSE,warning=FALSE,fig4.pos="H",fig.width=9,fig.height=6,fig.cap="\\label{fig:fig4} Different admixure duration models ranging from a one generation pulse tu 2500 generations for Neandertal admixture duration using all 1k Genome CEU individuals as the admixed population with all YRI and 3 high coverage Neandertals as reference populations."}
...
for(i in 1:length(t_d)){
  ...
  Real_Lomax_fit_Result <- c(Real_Lomax_fit_Result,fit.ggplot_lomax_x)
  Real_Lomax_fit_Result <- as.data.frame(Real_Lomax_fit_Result)
}
  Real_fit_Result <- c(Real_Lomax_fit_Result,fit.ggplot_exp)
  Real_fit_Result <- as.data.frame(Real_fit_Result)
  colnames_Real_fit_Result  <- c()
  for(t in t_d){
    colnames_Real_fit_Result <- c(colnames_Real_fit_Result,c(t,'Genetic_Distance'))
  }
  colnames_Real_fit_Result  <- c(colnames_Real_fit_Result,c(0,'Genetic_Distance'))
...
P_Gamma_2 <- Figure_1_C_2(c(1,t_d),5000,5000,1500,Colour_P = c("red",cbPalette_viridis),4,3100)
