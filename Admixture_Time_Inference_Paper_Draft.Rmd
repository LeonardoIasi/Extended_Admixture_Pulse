...
in this single generation can be modeled using a single exponential distribution. This however limits the information gained form the dating since in case of a 
...
These uncertainties in the genetic map were found to downward bias the time estimates by missing certain recombination events resulting in assuming introgressed segments longer than they actually are [@sankararaman_date_2012].  
<!-- Go a bit more into detail what is known about the bias -->
...
This is especially a problem for archaic admixture dating since the truly introgressed segments are in the same range length as the falsely identified ones [@sankararaman_date_2012]. Therefore ascertainment of variants informative of archaic introgression is used to enrich the signal of ALD to not conflate it with background LD from demographic processes which is in the same range length [@sankararaman_date_2012;@fu_genome_2014;@sankararaman_combined_2016;@moorjani_genetic_2016].
...
To clarify the effect of continuous admixture modeled as a pulse, we have to compare it to all the other potential biases. Moreover, to explicitly model continuous admixture and to distinguish a pulse like event from continuous we have to be aware of strong biases. Therefore we will estimate the effect sizes of all the assumptions on the mean time estimates for scenarios of Neandertal admixture.
...
While good estimates for the mean admixture time between Neandertals and modern humans exist, little is known about the duration of the event. In particular, dates when admixture ended are of crucial importance, as they are relevant to Neandertals extinction. Models were introduced to model multi-generation long admixture. If admixture is happening over $n$ generations, this results in a mixture of $n$ different exponential distribution, one for the segments entered in each generation [@pickrell_ancient_2014].  Previous studies tried to model this, using the ALD, by assuming not a continuous admixture but summarizing it into different independent events. In this case, the decay curve is comprised out of $n$ different exponential terms, each for one admixture event [@pickrell_ancient_2014; @zhou_inference_2017]. To model it as a continuous admixture Zhou et al. 2017 suggested using a polynomial function with the number of coefficients related to the number of generations of continuous admixture [@zhou_modeling_2017]. These models however work best with recent admixture, in the last 100 generations, with a substantial admixture proportion of around 30 % [@zhou_modeling_2017]. 
