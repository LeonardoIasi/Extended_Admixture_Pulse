...
Figure_1_A_Data$Gamma_Mean <- Get_gamma_mean(Figure_1_A_Data$GF_Start,Figure_1_A_Data$GF_Stop)
...
Recom_Sim_1$Gamma_mean <- Get_gamma_mean(Recom_Sim_1$GF_Start,Recom_Sim_1$GF_Stop)
...
Recom_Sim_2$Gamma_mean <- Get_gamma_mean(Recom_Sim_2$GF_Start,Recom_Sim_2$GF_Stop)
...
Recom_Sim_and_Inf_1$Gamma_mean <- Get_gamma_mean(Recom_Sim_and_Inf_1$GF_Start,Recom_Sim_and_Inf_1$GF_Stop)
...
Recom_Sim_and_Inf_2$Gamma_mean <- Get_gamma_mean(Recom_Sim_and_Inf_2$GF_Start,Recom_Sim_and_Inf_2$GF_Stop)
...
other. Figure \ref{fig:figS1} shows the two demographies of
...
(Supp. Fig. \ref{fig:figS2}). A GLM was applied to
...
Plot_model_result <- precis(Effect_size_fixed_s,pars = c("a","bG","bR","bD","bm","bA"),prob = 0.95)

ggplot(Plot_model_result,aes(y=as.character(row.names(Plot_model_result)),x=as.numeric(as.character(mean))))+
    ...
    geom_errorbar(aes(ymin=as.character(row.names(Plot_model_result)),ymax=as.character(row.names(Plot_model_result))),width=c( abs(as.numeric(as.character(Plot_model_result$`97.5%`))-as.numeric(as.character(Plot_model_result$`2.5%`)))),col="black")+
  geom_vline(xintercept = 0,linetype=2,aes(colour='grey'))+
  labs(x = "Standardized difference between true and estimated time")+
  labs(y = "")+
  theme(plot.title = element_text(hjust = 0.5,size = 12))+
  scale_y_discrete(labels= c("Standart Model","HES","Complex Demography","Continuous GF","d0 = 0.05","Recombination Map"))+
  scale_color_manual("Gene Flow Model",
...
Fig_3 <-  ggarrange(PX1,PX2,PX3,PX4,PX5,
          ncol = 5, nrow = 1)
annotate_figure(Fig_3,bottom = text_grob("Predictor combination"),left  = text_grob("Standardized difference between true and estimated time",rot = 90))
...
Result.Table.fn <- function(Result.Table.path,Sampling.time.from.GF.End,name){
  ...
  Result.Table$True_gamma_mean_GF <- Get_gamma_mean(Result.Table$GF.Start,Result.Table$GF.End)
...
Plot_sampling_50_gen_t_GF <- function(ggdata_t.GF,cbPalette_viridis,Ptitle){
  ggplot(ggdata_t.GF,aes(x=variable,y=as.numeric(as.character(value)),colour=factor(Name)))+
    ...
    geom_hline( aes(yintercept = True_gamma_mean_GF ))+
    ggtitle(Ptitle) +
...
ggdata_t.GF_50_mean_GF_exp <- melt(Plot.data_50,measure.vars =  c('mean_GF_exp'),id.vars = c('True_gamma_mean_GF','True_GF_length','Name'))
ggdata_t.GF_50_mean_GF_lomax <- melt(Plot.data_50,measure.vars =  c('mean_GF_lomax_s'),id.vars = c('True_gamma_mean_GF','True_GF_length','Name'))
ggdata_l.GF_50 <- melt(Plot.data_50,measure.vars =  c('length_GF'),id.vars = c('True_gamma_mean_GF','True_GF_length','Name'))
...
ggdata_t.GF_50_fixed_Lambda <- melt(Plot.data_50_fixed_Lambda,measure.vars =  c('mean_GF_lomax_s'),id.vars = c('True_gamma_mean_GF','True_GF_length','Name'))
ggdata_l.GF_50_fixed_Lambda <- melt(Plot.data_50_fixed_Lambda,measure.vars =  c('length_GF'),id.vars = c('True_gamma_mean_GF','True_GF_length','Name'))
...
ggdata_t.GF_varying_mean_GF_exp <- melt(Plot.data_varying,measure.vars =  c('mean_GF_exp'),id.vars = c('True_gamma_mean_GF','True_GF_length','Name','Sample_Time'))
ggdata_t.GF_varying_mean_GF_lomax <- melt(Plot.data_varying,measure.vars =  c('mean_GF_lomax_s'),id.vars = c('True_gamma_mean_GF','True_GF_length','Name','Sample_Time'))
ggdata_l.GF_varying <- melt(Plot.data_varying,measure.vars =  c('length_GF'),id.vars = c('True_gamma_mean_GF','True_GF_length','Name','Sample_Time'))
...
ggdata_t.GF_varying_fixed_Lambda <- melt(Plot.data_varying_fixed_Lambda,measure.vars =  c('mean_GF_lomax_s'),id.vars = c('True_gamma_mean_GF','True_GF_length','Name','Sample_Time'))
ggdata_l.GF_varying_fixed_Lambda <- melt(Plot.data_varying_fixed_Lambda,measure.vars =  c('length_GF'),id.vars = c('True_gamma_mean_GF','True_GF_length','Name','Sample_Time'))
...
New.P1_fixed_Lambda <- Plot_sampling_50_gen_t_GF(ggdata_t.GF_50_fixed_Lambda,cbPalette_viridis,"Sampled 50 Gen. From End Of Admixture")
New.P2_fixed_Lambda <- Plot_sampling_50_gen_GF_Length(ggdata_l.GF_50_fixed_Lambda,cbPalette_viridis,"Sampled 50 Gen. From End Of Admixture")
New.P3_fixed_Lambda <- Plot_sampling_varying_gen_t_GF(ggdata_t.GF_varying_fixed_Lambda,cbPalette_viridis,"Admixture Duration = 800 Gen.")
New.P4_fixed_Lambda <- Plot_sampling_varying_gen_GF_Length(ggdata_l.GF_varying_fixed_Lambda,cbPalette_viridis,"Admixture Duration = 800 Gen.")
```
...
using the exact same genetic map gives unbiased results. The mean time estimates received from the Lomax distribution show an overestimation on average and a higher variation (Supplement figure \ref{fig:figS3}).  Figure
...
```{r fig6, message=FALSE, echo=FALSE,warning=FALSE,fig6.pos="H",fig.width=9,fig.height=6,fig.cap="\\label{fig:fig5} Different admixure duration models ranging from a one generation pulse tu 2500 generations for Neandertal admixture using directly infered introgressed segments from Skov et al. 2020"}
### For the paper plot cutoff 0.05 - 0.5 cM
lower_trunc= 0.05
upper_trunc= 0.5
m=0.02
Ne=5000
Fragments <- read.table("Real_Data_Analysis/Archaicfragments_Iceland.txt",header = T)

Recomb_Map <- read.table("Real_Data_Analysis/RecombmapIceland.txt", header = T)

Fragments_Filtered <- Fragments[Fragments$archaic=="Vindija" | Fragments$archaic=="Altai",]

Assign_Genetic_distance_linear_fn <- function(Recomb_Map,Fragments){
  Fragments_cM <- c()
  for(chr in 1:23){
    if(chr==23){
      chr="X"
    ...
    Start_Arch_Freg <- approx(x=Recomb_Map_chr$Begin, y=Recomb_Map_chr$cM, xout=Fragments_chr$start,method = "linear")
    Stop_Arch_Freg <- approx(x=Recomb_Map_chr$Begin, y=Recomb_Map_chr$cM, xout=Fragments_chr$end,method = "linear")
...
fit_exp_optim_rep=function(Fragments,lower_trunc,upper_trunc,m,Ne){
  ...
  try({
    ...
    par =exp(res$par)
    t_m = (par*100)/(1-m)
    t_m_cor_Inb = log(1-((par*100)/(2*Ne*(1-m))))*-2*Ne
    return(data.frame(rate=par, ll_exp = -res$value, t_m= t_m ,t_m_cor_Inb = t_m_cor_Inb))
  }, silent=F)
  return(data.frame(rate=NA, ll_exp=NA, t_m= NA ,t_m_cor_Inb = NA))
}
...
fit_lomax_optim_rep=function(Fragments,fixed_shape,lower_trunc,upper_trunc){
  l=Fragments[Fragments>=lower_trunc & Fragments<=upper_trunc]
  n = rep(1, length(l))
  ...
  try({
    ...
    res = optim(c(-1), f, method="L-BFGS-B")
    pars = exp(res$par)
    return(data.frame(scale=pars,t_m=fixed_shape/pars,tm_exp=(fixed_shape/pars)*100, ll_lomax = -res$value,lower_trunc=lower_trunc,upper_trunc=upper_trunc))
  }, silent=F)
...
Expo_est=fit_exp_optim_rep(Fragments = Fragments_cM$length_cM,lower_trunc = lower_trunc,upper_trunc = upper_trunc,m = m,Ne = Ne)

t_d=c(1,100,200,400,800,1000,1500,2000,2500)
T_lomax_est <- c()
for(t in 1:length(t_d)){
  fixed_shape= (Expo_est$t_m_cor_Inb^2)/((t_d[t]/4)^2)
  Res_Segments_optim_lomax=fit_lomax_optim_rep(Fragments_cM$length_cM,fixed_shape,lower_trunc,upper_trunc)
  Res_Segments_optim_lomax$fixed_shape <- fixed_shape  
  T_lomax_est <- rbind(T_lomax_est,Res_Segments_optim_lomax)
}
T_lomax_est$duration <- t_d
l=Fragments_cM$length_cM[Fragments_cM$length_cM>=lower_trunc & Fragments_cM$length_cM<=upper_trunc]
Plot_Fragments <- hist(l,plot = F,breaks = 1000)
Plot_Fragments <- data.frame(mids=Plot_Fragments$mids,density=Plot_Fragments$density)
Expo_fit=data.frame(y=dtexp(Plot_Fragments$mids,Expo_est$rate,lower_trunc ,upper_trunc),x=Plot_Fragments$mids)
Lomax_fit <- c()
for(i in 1:length(T_lomax_est$scale)){
  Lomax_fit_i=data.frame(y=dtlomax(x = Plot_Fragments$mids,scale = T_lomax_est[i,1],shape = T_lomax_est[i,7],lower_trunc = lower_trunc,upper_trunc = upper_trunc)
                         ,x=Plot_Fragments$mids)
  Lomax_fit <- rbind(Lomax_fit,Lomax_fit_i)
}

Icelandic_Fit_Results=rbind(Expo_fit,Lomax_fit)
Icelandic_Fit_Results$Duration <- as.factor(rep(c(0,t_d),each=length(Expo_fit$x)))


ggplot(data=Icelandic_Fit_Results,aes(x = , y = y,color=Duration,fill=Duration))+
  geom_point(data=Plot_Fragments,aes(x=mids,y=log(density),color=NULL,fill=NULL),show.legend = F)+
  geom_line(data=Icelandic_Fit_Results,aes(x = x, y = y,color=Duration))+
  scale_color_manual("Admixture Duration",
...
```{r figS3, message=FALSE, echo=FALSE,warning=FALSE,fig.width=12,fig.height=6,fig.cap="\\label{fig:figS3} Lomax estimates for the mean time."}

New.P1_mean_GF_exp <- Plot_sampling_50_gen_t_GF(ggdata_t.GF_50_mean_GF_exp,cbPalette_viridis,"Sampled 50 Gen. from End Of Admixture Exponential Fit")
New.P1_mean_GF_lomax <- Plot_sampling_50_gen_t_GF(ggdata_t.GF_50_mean_GF_lomax,cbPalette_viridis,"Sampled 50 Gen. from End Of Admixture Lomax Fit")
New.P3_mean_GF_exp <- Plot_sampling_varying_gen_t_GF(ggdata_t.GF_varying_mean_GF_exp,cbPalette_viridis,"Admixture Duration = 800 Gen. Exponential Fit")
New.P3_mean_GF_lomax <- Plot_sampling_varying_gen_t_GF(ggdata_t.GF_varying_mean_GF_lomax,cbPalette_viridis,"Admixture Duration  = 800 Gen. Lomax Fit")
