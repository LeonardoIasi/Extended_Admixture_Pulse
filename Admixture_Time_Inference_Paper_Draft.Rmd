...
A_pred <- link( Effect_size_fixed_s , data=data.frame(A=seq(0,1,0.05),MD=0,D=0,R=0,GF=0) )
A_pred.mean <- apply( A_pred , 2 , mean )
A_pred.PI <- apply( A_pred , 2 , PI , prob=0.95 )
boxplot(rep(as.numeric(as.character(Simplest_case$Diff_s)),Simplest_case$Freq),
      rep(as.numeric(as.character(Plot_model$Diff_s[Plot_model$Combination=='Simple_D_Sim_HES_0_P'])),Plot_model$Freq[Plot_model$Combination=='Simple_D_Sim_HES_0_P']), 
      col=col.alpha(rangi2,0.5),ylim=c(-0.5,0.5) )
# plot the MAP line, aka the mean mu for each weight
lines( seq(1,2,0.05) , A_pred.mean,add=T )
# plot a shaded region for 89% PI
shade(A_pred.PI ,seq(1,2,0.05),add=T)
...
Plot_Fig_2_X <- function(Null_Case,Predictore_case,B_Model,Int,Pred_data,Slope,x.lable,x.title,xlab=NULL,ylab=NULL,y.ticks=F){
  Factor_pred <- link( Effect_size_fixed_s , data=data.frame(A=Pred_data[1],MD=Pred_data[2],D=Pred_data[3],R=Pred_data[4],GF=Pred_data[5]) )
  Factor_pred.mean <- apply( Factor_pred , 2 , mean )
  Factor_pred.PI <- apply( A_pred , 2 , PI , prob=0.95 )
  pred_Data <- data.frame(x=seq(0,1,0.05),y=Factor_pred.mean)
  Data=rbind(Null_Case,Predictore_case)
  if(y.ticks==F){
    Px <-  ggplot(Data,aes(x=as.character(Combination),y=as.numeric(as.character(Diff_s)),colour=Col))+
      geom_point(aes(size = Freq),show.legend = F)+
      geom_boxplot(show.legend = F)+
      geom_abline(data=pred_Data,aes(x=x,y=y),linetype=2)+
      #geom_abline(seq(1,2,0.05) , Factor_pred.PI[,1])+
      #geom_abline(seq(1,2,0.05) , Factor_pred.PI[,2])+
      geom_abline(intercept = 0,slope = 0,linetype=2,aes(colour='grey'))+
      labs(x = xlab)+
      labs(y = ylab)+
      scale_x_discrete(labels= x.lable)+
  ...
  if(y.ticks==T){
    Px <-  ggplot(Data,aes(x=as.character(Combination),y=as.numeric(as.character(Diff_s)),colour=Col))+
      geom_point(aes(size = Freq),show.legend = F)+
      geom_boxplot(show.legend = F)+
      geom_line(data=pred_Data,aes(x=pred_Data[,1],y=pred_Data[,2]),linetype=2)+
      #geom_abline(seq(1,2,0.05) , Factor_pred.PI[,1])+
      #geom_abline(seq(1,2,0.05) , Factor_pred.PI[,2])+
      geom_abline(intercept = 0,slope = 0,linetype=2,aes(colour='grey'))+
      labs(x = xlab)+
...
PX1 <- Plot_Fig_2_X(Simplest_case,Plot_model[Plot_model$Combination=='Simple_D_Sim_HES_0_P',],B_model_result,c(seq(0,1,0.05),0,0,0,0),Int = 1,Slope = 6
             ,x.lable = c("LES","HES"),"Ascertainment\nScheme",y.ticks = T)
