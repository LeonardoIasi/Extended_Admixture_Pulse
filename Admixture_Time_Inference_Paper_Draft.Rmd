...
Figure_1_C_2 <- function(Timespan_GF_Models,time_l,Split_time,Mean_Time,Colour_P,max_y,max_x){
  ...
  Px <-  ggplot(data =GF.melted, mapping=aes(x = Time, y = value,color=variable)) +
    geom_area(position = 'identity',mapping = aes(x = ifelse(Time>Mean_Time-(as.numeric(as.character(variable))/2) & 
                                                               Time< Mean_Time+(as.numeric(as.character(variable))/2) , Time, NA),
                                                  fill=variable),show.legend = F,alpha = 0.33) +
    geom_line(show.legend = F)+
    coord_cartesian(ylim = c(0,2.5e-04),xlim = c(0,3050),expand = 0)+
    geom_vline(xintercept=Split_time,linetype=3,color='black')+
    scale_color_manual(values = Colour_P)+
...
t_d=c(1,100,200,400,800,1000,1500,2000)
...
ggplot(Post_effect_size_est,aes(x=row.names(Post_effect_size_est),y=as.numeric(as.character(mean))))+
  ...
  scale_x_discrete(labels= c("Standard Model","Continuous GF","Variying recombination","Complex Demography","d0=0.02 cM","HES")
...
Plot_sampling_50_gen_t_GF <- function(ggdata_t.GF,cbPalette_viridis,Ptitle){
  ggplot(ggdata_t.GF,aes(x=variable,y=as.numeric(as.character(value)),colour=factor(Name)))+
    ...
    scale_color_manual("Simulations",
                       ...
                       labels = c("Constant Recombination","HapMap not corrected","HapMap AAMap corrected","HapMap HapMap corrected"))
...
ggdata_reodering_fn <- function(ggdata,odering){
  ggdata$Name <- factor(ggdata$Name,
    levels = odering,ordered = TRUE)
  return(ggdata)
}

New.P1_mean_GF_exp <- Plot_sampling_50_gen_t_GF(ggdata_reodering_fn(ggdata_t.GF_50_mean_GF_exp,c("Recent_50_constant","Recent_50_HapMap_no_correction","Recent_50_HapMap_AAMap_correction","Recent_50_HapMap_HapMap_correction")),cbPalette_viridis, "Sampled 50 Gen. From End Of Admixture")
New.P1_mean_GF_lomax <- Plot_sampling_50_gen_t_GF(ggdata_reodering_fn(ggdata_t.GF_50_mean_GF_lomax,c("Recent_50_constant","Recent_50_HapMap_no_correction","Recent_50_HapMap_AAMap_correction","Recent_50_HapMap_HapMap_correction")),cbPalette_viridis,"Sampled 50 Gen. From End Of Admixture")
New.P2 <- Plot_sampling_50_gen_GF_Length(ggdata_reodering_fn(ggdata_l.GF_50,c("Recent_50_constant","Recent_50_HapMap_no_correction","Recent_50_HapMap_AAMap_correction","Recent_50_HapMap_HapMap_correction")),cbPalette_viridis,"Sampled 50 Gen. From End Of Admixture")

New.P3_mean_GF_exp <- Plot_sampling_varying_gen_t_GF(ggdata_reodering_fn(ggdata_t.GF_varying_mean_GF_exp,c("Recent_50_constant","Recent_50_HapMap_no_correction","Recent_50_HapMap_AAMap_correction","Recent_50_HapMap_HapMap_correction")),cbPalette_viridis,"Admixture Duration = 800 Gen.")
New.P3_mean_GF_lomax <- Plot_sampling_varying_gen_t_GF(ggdata_reodering_fn(ggdata_t.GF_varying_mean_GF_lomax,c("Recent_50_constant","Recent_50_HapMap_no_correction","Recent_50_HapMap_AAMap_correction","Recent_50_HapMap_HapMap_correction")),cbPalette_viridis,"Admixture Duration = 800 Gen.")
New.P4 <- Plot_sampling_varying_gen_GF_Length(ggdata_reodering_fn(ggdata_l.GF_varying,c("Recent_50_constant","Recent_50_HapMap_no_correction","Recent_50_HapMap_AAMap_correction","Recent_50_HapMap_HapMap_correction")),cbPalette_viridis,"Admixture Duration = 800 Gen.")
...
Constant_sampling_time <- ggarrange(Mean_GF_F_50,Duration_GF_F_50,
...
# Plot Together
Extended_Pulse_Evaluation <- ggarrange(Constant_sampling_time,Varying_sampling_time,nrow = 2,labels = c("A","B"),common.legend = T,legend = 'bottom')
