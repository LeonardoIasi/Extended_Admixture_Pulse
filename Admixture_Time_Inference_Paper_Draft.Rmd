...
In addition, admixture is recognized as shape genetic diversity and can
...
generatin pulse. By relating this effect to other modelling assumptions,
...
assumotion to obtain the duration of an admixture event.
...
challenge is to correctly infer the length of the introgressed segments.
...
admixture however, an ascerteinment scheme is used to calculate a
pairwise LD between introgressed variants. The ascerteinment scheme
filters for introgressed variants in the admixed population, since LD
form admixture and LD from other processes is in a similar length range
...
be 37,000--86,000 ya (years ago) (\cite{sankararaman_date_2012}). Later,
this data was refined to 40,510--54,454 ya (95\% CI) using a different
...
admixture was dated to be between 44,000--54,000 ya using the ALD
approach on modern day genomes (\cite{sankararaman_combined_2016}).
Similarly, the identifcation of denisovan segments private to
South-East-Asian population, which are more diverged from the Denisovan
high-coverage genome then previously found segments, suggests an
additional admixture event between these populations
(\cite{browning_analysis_2018}).

Comparing the mean length of the two groups of segments introgressed
...
admixture events with Denisovan, one in line with previous estimates at
...
variants, and admixture times. Crucially, even if the length estimation
is unbiased, the admixture time estimates can be substantially biased
(\cite{sankararaman_date_2012,fu_genome_2014}). There are different
recombination maps used to assign the genetic length between
introgressed segments/variants available
(\cite{coop_high-resolution_2008,kong_fine-scale_2010,hinch_landscape_2011,wegmann_recombination_2011,mcvean_fine-scale_2004,myers_fine-scale_2005,HapMapConsortium_second_2007}).

They either focus on short term information by using parent-offspring
...
continuous admixture on the admixture time estimates in comparison to
...
\subsection{Simulations}\lable{simulations}
...
\subsubsection{Gene Flow}\lable{gene flow}
...
\subsubsection{Recombination map}\lable{recombination map}
...
\subsubsection{Inferred demography}\lable{inferred demography}
...
\subsection{Admixture time estimates}\lable{admixture time estimates}

\subsubsection{Ascertainment scheme}\lable{asceteinment scheme}
...
\subsubsection{ALD calculation and curve fitting}\lable{ALD calculation and curve fitting}
...
\subsubsection{Modeling parameter effect sizes}\lable{modeling prameter effect sizes}
...
\section{Results}\lable{results}

\subsection{Theoretical framework for continuous admixture}\lable{theoretical framework for continuous admixture}
...
\subsection{The effect of continuous admixture on the admixture time estimates}\lable{the effect of continuous admixture on the admixture time estimates}
...
\subsection{Comparing effect sizes}\lable{comparing effect sizes}
...
\subsection{Estimating the Lomax-parameters under different conditions}\lable{estimating the Lomax-parameters under different conditions}
...
\section{Discussion}\lable{discussion}
...
\section{Supplement}\lable{supplement}
