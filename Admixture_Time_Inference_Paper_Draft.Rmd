...
Here, we perform a simulation study where we  investigate the effect of model 
and data assumptions on estimated admixture  dates. We show that in many cases
...
*li 27/02: now I first explain what is the problem with genetic maps are and then how it affects the estimate according to Siriam (Prya quotes him on this) and then what is the problem with the correction*

Recombination maps are used to assign the genetic length between introgressed segments/variants. Different maps are available[@coop_high-resolution_2008; @kong_fine-scale_2010;@hinch_landscape_2011; @wegmann_recombination_2011;@mcvean_fine-scale_2004; @myers_fine-scale_2005; @hapmap_consortium_2007]. 
...
changes *ref Gravel2012?*, demographic processes can create similar genomic signals as 
admixture *how*. Genomic segments carrying these signals can falsely be included 
in the estimate of the length distribution of admixed segments, leading to a bias.  *how?*
Admixture segments can also get lost by genetic drift, making it harder to 
estimate the length distribution.*why?* 
...
An additional bias might be introduced in the ALD based dating method since it requires 
ascertaining the SNPs for admixture informative sites, especially for archaic 
admixture [@sankararaman_date_2012; @fu_genome_2014]. 
...
time between start end end of the gene flow. Ideally one would like to obtain 
the start and end, hence the duration of admixture between Neandertals and non-African populations. 

*bp 19/02: preceeding section could be merged with last intro paragraph*

If admixture is happening over $n$ generations, this results in a mixture of $n$ 
