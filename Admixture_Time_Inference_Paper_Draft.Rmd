...
To distinguish a pulse like admixture event resulting in an exponetially distributed ALD decay from a Lomax distributed ALD decay accosiated with a continuous admixture, we compared the two model via goodness-of-fit to the ALD data. Therefore we used Akaike's information criterion comparing the exponetial model nested in the Lomax. We expect that, since the AIC is penalizing extra parameter, that a Lomax fit to a pulse like simulation scenario should be rejected, wheres for all the other scenarios the Lomax would be prefered. However, for the simplified simulation scenarios under a constant recombination rate were we obtained the best results the Lomax displayes a lower AIC in `r Pulse_model_selction[,2]` fits out of 100 replications. Model comparison for scenarios of true continuous admixture always resuted in the Lomax having a lower AIC. The AIC reject the null hypotithis to often and thus seem to be a to liberal test.
...
Conclusion:
  - even under a constant rate the signal of continuous gene flow for scenarios of Neandertal admixture are probbably not detactable any more since one samples to far away from the end of the gene flow event.
  - the ALD decay is approximated assuming a constant rate of recombination, this works fairly well for mean time estimates with a precise map but duration estimates are very sensitive to this violation.
  - Comparisons between the two nested models via Akaike's infromation criterion always preferes the Lomax fit, even in the pulse admixture scenario, where the Lomax reflects the exponantial distribution and thus the extra parameter in the Lomax should be punished. 
