...
#Introduction
*bp, li 28/02: in generall motivate why you are talking about stuff, why is it important for our question and dont go into explenaition straight away*

Detecting admixture, i.e. gene flow between previously isolated populations can shed light into the complex evolutionary history of populations. Admixture, sometimes also referred to as hybridization, is an important factor of speciation and occurs in almost all proposed processes of it [@abbott_hybridization_2013]. In its most basic understanding, the capacity to admix is used as a criterion to characterize species, as in the biological species concept a species is defined as reproductively isolated from others [@mayr_animal_1963]. 

<!-- This classical allopatric concept of speciation, however is questioned by the emergent finding of speciation events in the presence of admixture [@sousa_understanding_2013].  -->
<!-- Not sure about this sentence this is not the topic of the paper -->

In addition admixture is recognized to shape genetic diversity and can introduce novel variants into the admixing populations [@harrison_hybridization_2014]. These variants can be adaptive in the admixed population [@hedrick_adaptive_2013] or be selected against at loci which determine species specific phenotypes [@shaw_genes_2011], thereby helping to identify loci important for local adaptation [@payseur_using_2010]. 
<!-- Admixture is found in many sexually reproducing species of plants, invertebrates and vertebrates [@payseur_genomic_2016].  -->
Novel DNA sequencing methods make it easier to detect admixture event between populations [@sousa_understanding_2013]. The sequencing of the Neandertal [@green_draft_2010] and Denisovan [@reich_genetic_2010] genome revealed admixture events between Neandertals and  modern humans outside of Africa  [@green_draft_2010; @prufer_complete_2013; @vernot_resurrecting_2014;@fu_early_2015; @fu_genome_2014; @sankararaman_genomic_2014;@prufer_high-coverage_2017], as well as a second admixture event between Denisovans and Asian populations [@reich_genetic_2010; @meyer_high-coverage_2012; @sankararaman_combined_2016; @vernot_excavating_2016]. These admixture events where found to have impacted modern human phenotypes [@dannemann_something_2018]. One important question is the date and duration of such admixture events, since this can be a clue to the debated question of the time of extinction of these hominin populations.
...
*bp, li 28/02:artbildung hervorheben wieso sind wir interresiert daran biologisch nicht sofort mea teschnisch wreden i.e. why do we care about admixture at all*



##Admixture on the genomic level and basic idea how to date it

On the genomic level, admixture introduces highly divergent chromosomal segments into the admixing population. Over time, miotic recombination progressively breaks these introgressed segments down [@falush_inference_2003].
Assuming that recombination events occur according to a Poisson process, the segment length distribution for segments entered in the same generation, can be approximated by an exponential distribution, whose rate parameter is informative about the number of generations since the segments entered the admixing population [@moorjani_history_2011;@pool_inference_2009; @gravel_population_2012].
In particular, under a constant recombination rate, the length of introgressed segments should be inversely proportional to the time since the admixture event [@gravel_population_2012; @liang_lengths_2014].  Hence, the length distribution of introgressed chromosomal segments ('recombination clock') can be used to inferred the time since the admixture event [@moorjani_history_2011;@pugach_dating_2011; @sankararaman_date_2012;@loh_inferring_2013;@sankararaman_combined_2016;@pugach_gateway_2018;@jacobs_multiple_2019,@hellenthal_genetic_2014]. To do this, it is usually assumed that admixture happened as an admixture pulse [@moorjani_history_2011], where admixture between two populations is only a one generation long event followed by a period of isolation [@liang_lengths_2014]. This single pulse is mathematically convenient since all the introgressed segments entering 
in this one generation can be modeled using a single exponential distribution [ref?]. This however limits the information gained from the dating since in case of a 
multi-generation long admixture the single-pulse model results in a mean admixture 
time between start and end of the admixture. Ideally one would like to obtain the start and end, hence the duration of admixture between populations.


*bp 19/02: Maybe introduce here the term of admixture pulse*

*bp,li 28/02: Hervorheben der main idea das wir die laenge vom Nean gf messen wollen especially das ende die hauptfage hierbei waere inwieweit das end von einem gf mit dem bisher gemessenen mittelwert entfernt ist. Da wir wissen das der mittelwert durch die ganzen untengennanten faktoren beeinflusst werden kann muessen wir diese auch fuer die laengen messung in betracht ziehen*


##The two approaches of obtaining the length of introgressed segments: ALD
To obtain the date of admixture, introgressed segments have to be identified.
Since admixture segments are not directly observable, the challenge is to correctly infer the length of the introgressed segments. There are two main approaches to identify segments for dating admixture  [@chimusa_dating_nodate] (Figure \ref{fig:Intro_fig} A). 

The first approach uses the admixture-induced linkage disequilibrium (ALD) decay. 
Admixture introduces highly divergent archaic segments into the human background. Hence, variants on these archaic segments are expected to be in high linkage disequilibrium to each other at the time of admixture [@chakraborty_admixture_1988; @stephens_mapping_1994; @wall_detecting_2000]. 
As recombination breaks the introgressed chromosomal segments apart each generation, the association between variants on these introgressed segments decays as genetic distance increases. In case of a recent admixture event, ALD is found over long genetic distances [@patterson_methods_2004] and is therefore easily distinguishable from short range non-ALD [@moorjani_history_2011]. For ancient admixture however, a pairwise LD between introgressed variants is calculated using an ascertainment scheme to filter for the introgressed variants in the admixed population, since LD form admixture and LD from other processes is in a similar length range [@sankararaman_date_2012]. 
<!-- does not to seem clear how ascertainment works -->
The pairwise LD as a function of genetic distance is then fitted using an exponential distribution holding a point estimate for the mean time since the admixture event [@moorjani_history_2011; @loh_inferring_2013]. 


##The two approaches of obtaining the length of introgressed segments: searching for introgressed haplotypes
The other approach first tries to infer introgressed haplotypes directly, by partitioning a genome into admixture segments [@gravel_population_2012; @price_sensitive_2009; @lawson_inference_2012]. Multiple methods are available focusing on different characteristics of introgressed haplotypes, such as LD patterns and density of derived variants, to infer them [@lawson_inference_2012; @racimo_signatures_2017; @seguin_orlando_paleogenomics_2014; @vernot_excavating_2016; @sankararaman_combined_2016; @skov_detecting_2018]. In the second step, the length distribution of these directly inferred introgressed haplotypes can be used for dating by also fitting an exponential distribution holding a point estimate for the time since the admixture event.
*Alba 09/03: Diffrence not clear between the two methods*
...
Both approaches have been used to estimate the time of admixture between Neandertals and modern humans assuming a one generation admixture pulse.  Using the decay of introgressed neandertal variants in modern day genomes, Sankararaman et al dated the neandertal-human admixture time to be 37,000–86,000 ya (years ago) [@sankararaman_date_2012]. Later, this data was refined to 40,510–54,454 ya (95% CI) using a different ascertainment scheme combined with a different genetic map [@moorjani_genetic_2016]. A date was also obtained from an ancient genome to be 50,000 - 60,000 ya by adding the admixture date of the ancient individual, obtained by the decay of pairwise covariance between introgressed SNPs, to the specimens radiocarbon date [@fu_genome_2014]. 

An additional admixture event between Neandertals and East-Asian populations at the same time as the admixture between Neandertals and all non-Africans was suggested to explain the higher amount of neandertal ancestry in present-day East Asians [@kim_selection_2015; @vernot_complex_2015]. The Denisovan human admixture was dated to be between 44,000–54,000 ya using the ALD approach on modern day genomes [@sankararaman_combined_2016]. 
Similarly, the identifcation of denisovan segments private to East-Asian population, which are more diverged from the Denisovan high-coverage genome then previously found segments, suggests an additional admixture event between these populations [@browning_analysis_2018].
<!-- Similarly, an additional Denisovan admixture event was suggested for East-Asian populations by directly identifying Denisovan segments, which are more diverged from the Denisovan high-coverage genome then previously found segments [@browning_analysis_2018]. -->

Comparing the mean length of the two groups of segments introgressed from divergent Denisovan populations did, however, not reveal significant differences, suggesting a lack of power to distinguish the two events by time [@browning_analysis_2018; @jacobs_multiple_2019]. Analysing genomes from Papuan individuals revealed two time separated admixture events with Denisovan, one in line with previous estimates at 45.7 kya (95% CI 31.9-60.7 kya) and one exclusive to Papuans dated to be around 29.8 kya (95% CI 14.4-50.4 kya) [@jacobs_multiple_2019].

<!-- All these studies focused on the mean time estimates of admixture, but preferentially one would like to obtain the duration. Especially the end of gene flow between modern humans and archaic hominin populations is of great interest.  -->
All these studies focused on the mean time estimates of admixture, but in this case the end of gene flow between modern humans and archaic hominin populations is of special interest. 
The effect of modeling a multi-generation long admixture as a one-generation pulse remains to be studied.
 

How far apart can the time of the end of admixture be from its mean time to still get reliable estimates, i.e. is a one-generation pulse distinguishable from a multi-generation continuous admixture event? If the difference is not large and a pulse-like event easily distinguishable, estimating the mean time alone is highly informative of the whole admixture timeline. If there would be a big difference instead and the pulse and continuous event could not be teased apart, the timeline of the admixture and its implication on the date of extinction of the hominin populations is highly uncertain.

*Alba 09/03: You already mentioned this in the Introduction section 1, and I like it how you did it there better, it “sold“ better the importance of the question.*
...
All dates obtained have to be based on a priori knowledge and assumptions.
<!-- Both approaches to identify the introgressed segments, the direct measure of the segment length and the indirect ALD based method, without first directly inferring introgressed segments such as the popular ALDER method [@loh_inferring_2013], rely on some fairly strong modelling assumptions [@pool_inference_2009; @gravel_population_2012; @liang_lengths_2014]. -->
Both approaches to identify the introgressed segments rely on some fairly strong modelling assumptions [@pool_inference_2009; @gravel_population_2012; @liang_lengths_2014].
Additional to the one-generation pulse simplification, the model assumes that the proportion of admixture of individuals in the admixed population is identical, tracts are rare and inbreeding is not significant, such that admixture segments do not recombine with each other to form longer segments [@pool_inference_2009]. The effect of drift is assumed to be negligible [@loh_inferring_2013], the introgressed segments act neutral [@shchur_distribution_2019] and the recombination map is identical across populations and does not change over time [@gravel_population_2012].

Violations of these assumptions are known to influence the mean time estimates. Hence, if we want to investigate the effect of assuming a one generation pulse on a multi-generation continuous admixture, we have to do this in comparison with the other biases. Moreover, to model continuous admixture we have to be aware of biasing effects. Therefore, we want to investigate the bias of each assumption and if some can be relaxed without causing a strong bias in the estimate.
...
*li 27/02: from liang et al 2014: Tract length can be correlated due to the variance in migrant ancestors an individual has and due to inbreeding joining together migrant tracts which violates the iid assumption especially in the cases mentioned* 
...
assumed not to recombine with each other to form longer segments. This assumption is violated in particular 
when admixture is very old $(t \approx 2N)$ or very recent $t \approx \log(2N)$ or when the admixture proportions are large [@liang_lengths_2014]. In our case we deal with small admixture proportions and $\log(2N) < t < 2N$.
...
<!-- Explain why recombination map matters so much -->

Second, it is assumed that the recombination rates are known. 
We know that the recombination process does not follow a strict Poisson process with recombination events occurring on a constant rate. Rather there are hotspots of high recombination throughout the genome [@mcvean_fine-scale_2004]. Hence, the physical length of an introgressed segment does not simply correlate to its genetic distance, therefore a recombination map is used to assign a genetic distance to the segments.
Since the recombination is used as a clock, misspecification of the recombination map will lead to over/or 
underestimates of segment lengths, and admixture times. Crucially, even if the length estimation is unbiased, the admixture time estimates can be substantially
biased [@sankararaman_date_2012;@fu_genome_2014]. There are different recombination maps used to assign the genetic length between introgressed segments/variants available[@coop_high-resolution_2008; @kong_fine-scale_2010;@hinch_landscape_2011; @wegmann_recombination_2011;@mcvean_fine-scale_2004; @myers_fine-scale_2005; @hapmap_consortium_2007]. 

*Alba 09/03 : I had to read it twice to realize that the types of map are not depending on application, but rather how they are obtained.*

They either focus on short term information by using parent-offspring data to directly observe recombination events [@coop_high-resolution_2008; @kong_fine-scale_2010], searching for admixture tracts breakpoints [@hinch_landscape_2011; @wegmann_recombination_2011] or long term population averages of recombination inferred by LD [@mcvean_fine-scale_2004; @myers_fine-scale_2005; @hapmap_consortium_2007]. 
Different genetic maps are highly correlated on a megabase scale but reveal significant population differences on a finer scale [@kong_fine-scale_2010; @hinch_landscape_2011]. The average length of neandertal introgressed segments is approximately 50 kb [@sankararaman_date_2012;@vernot_resurrecting_2014], and so fine-scale recombination maps are essential. 
Furthermore, recombination maps can also vary over time [@auton_fine-scale_2012]. Highly precise maps are not available for every population and the change through time is unknown. 
These uncertainties in the genetic map were found to downward bias the neandertal-human admixture time estimates by missing certain recombination events resulting in assuming introgressed segments longer than they actually are [@sankararaman_date_2012]. Using simulation studies deploying a varying recombination rate but assuming a constant rate for the neandertal admixture scenario, it was shown that dates were estimated around 20 % younger.
Sankararaman et al. 2012 suggested a method for accounting for some uncertainty in the recombination map by estimating a correction parameter for a given recombination map by comparing the distances between a pair of markers in the map to the number of crossovers that span those markers as observed in a pedigree map. This however limits the applicability of the method, since it requires a population-specific map as well as a pedigree based map to estimate the error in the map [@sankararaman_date_2012; @fu_genome_2014; @sankararaman_combined_2016 ; @moorjani_genetic_2016]. 
...
### Demography
*bp 19/02: this section (and frequently your writing in general) could need more structure. One simple approach is the   context-content-conclusion system For this intro section, the basic idea is that for each paragraph you need to make clear that you start by*
...
known. 
<!-- While the recombination clock itself is relatively robust to population size  -->
<!-- changes  -->

<!-- *bp 19/02: ref Gravel2012? li 27/02: No they didn't check for fluctuations in Ne* -->

<!-- , the problem is the detection of truly introgressed segments. Demographic processes can create similar genomic signals as admixture [@vernot_resurrecting_2014; @hsieh_model_based_2016]. These segments also exhibit high density of divergent variants in high LD to each other. There are multiple demographic processes that could lead to such signals.  -->
This is relevant because while demography does not alter the recombination clock significantly, it can create false signals of admixture [@vernot_resurrecting_2014; @hsieh_model_based_2016], by generating segments rich in divergent variantas in high LD to each other.
Figure \ref{fig:Intro_fig} B shows two possible demographies for the neandertal admixture, a simplified one without population size changes and a more complex one 
including structured populations, bottlenecks and  an additional admixture event. 
...
population, where human populations in Africa are subdivided with a low migration 
...
segments [@nei_linkage_1973;@pfaff_population_2001]. Genomic segments carrying these signals can falsely be identified as introgressed and included in the estimate of the length distribution of admixed segments, leading to a bias by considering them while fitting them to a distribution. That is because the signals from demography are usually short [@patterson_methods_2004;@moorjani_history_2011] (in terms of the physical length of the segment or range of LD), including them in the fitting process would lead to an excess of short segments/ALD shifting the distribution towards an older date.
This is especially a problem for archaic admixture dating since the true introgressed segments are in the same length range as the falsely identified ones [@sankararaman_date_2012]. Therefore ascertainment of variants informative of archaic introgression is used to enrich the signal of ALD to not conflate it with background LD from demographic processes [@sankararaman_date_2012;@fu_genome_2014;@sankararaman_combined_2016;@moorjani_genetic_2016]. Despite using an ascertainment scheme, Fu et al. 2014 showed a strong downward bias of around 50 % for simulations of neandertal admixture scenarios dated from modern human genomes with a complex demographic model [@fu_genome_2014].
Besides this, admixture segments can also get lost by genetic drift, resulting in less data and noisier fitting. Moreover an additional admixture event as shown in Figure \ref{fig:Intro_fig} B after the neandertal admixture event between the African and admixed non-African population was shown to potentially influence the detected amount of neandertal ancestry [@petr_limits_2019] as well as the time estimates [@sankararaman_date_2012]. 


### Methodological bias through ascertainment 

<!-- Better explain why (to disentengle non_ALD from ALD) -->
The ascertainment of SNPs for admixture informative sites in the ALD based dating methods might introduce a bias in itself.
Here, only positions in the admixed population are considered where both source populations are diverged from each other. The problem is that introgressed variants from the ancestral archaic 
population are ascertained using only a few sequenced archaic individuals as 
proxies for the actual introgressing archaic population [@sankararaman_date_2012;@fu_genome_2014;@sankararaman_combined_2016;@moorjani_genetic_2016]. 

*Alba 09/03: But that‘s always going to be the case, right? It seems unfair to put it with the other limitations without specifying that your model will also suffer from it. Maybe it‘s best to save it for the discussion.*

These individuals are potentially only distantly related to the introgressing population [@jacobs_multiple_2019], which might 
result in missing introgressed variants. 

*li 27/02 What to cite for the last sentence?*

###What are we going to do about all the assumptions
<!-- Not finish  -->

Using simulations, the effect of each assumption is obtained and explicitly modeled conditioning on all other assumptions to get individual effect sizes. Knowledge about a strong biasing effects will help to model continuous admixture, which is mathematically more challenging than a pulse admixture and the fitting process is more prone to interference by biasing effects.
...
<!-- While good estimates for the mean admixture time between Neandertals and modern humans exist, little is known about the duration of the event. In particular, dates when admixture ended are of crucial importance, as they are relevant to Neandertals extinction.  -->

Models were introduced before to study multi-generation long admixture.  If admixture is happening over generations, this results in a mixture of different exponential distribution, each one for the segments entered in each generation [@pickrell_ancient_2014].  Previous studies tried to model this, using the ALD, by assuming not a continuous admixture but summarizing it into different discrete events. In this case, the decay curve is comprised out of $n$ different exponential terms, each for one admixture event [@pickrell_ancient_2014; @zhou_inference_2017]. To model it as a continuous admixture Zhou et al. 2017 suggested using a polynomial function with the number of coefficients related to the number of generations of continuous admixture [@zhou_modeling_2017]. These models however work best with recent admixture, in the last 100 generations, with a substantial admixture proportion of around 30 % [@zhou_modeling_2017].
...
This is outside of the range for scenarios of neandertal admixture, were the admixture proportion is around 3 % and the timing around 1500 generations ago [@sankararaman_date_2012;@prufer_complete_2013;@sankararaman_genomic_2014]. 

Here we introduce a model for continuous admixture, where the migration rate over time is Gamma distributed with only two parameters, one for the mean time of continuous admixture and one for the duration of the admixture. In our simulation study, we first examine the effect of long continuous admixture on the admixture time estimates in comparison to the effects of the aforementioned model assumptions. 
<!-- Therefore, we quantitatively evaluate the effect of the admixture model, recombination, demography and analysis parameters on the inference.  -->
Second, we define the expectation of the resulting segment length distribution for continuous Gamma distributed admixture being Lomax distributed, holding a parameter for the duration of admixture. This expectation works for both methods to infer the segments length, either directly or by using the ALD decay. Using this model, we investigate under which scenarios the parameters of the Lomax-distribution can be  accurately  estimated and for which parameters we can distinguish a pulse-like admixture event from a continuous event. We show that in many cases
pulses cannot be distinguished from more continuous admixture events, and so current methods are unsuitable to definitively answer when the contact between 
...
Bdata <- list(
  E = standardize(xdata.M.3_no_error$Diff),
  A = ifelse(xdata.M.3_no_error$Ascertained==0,1,2),
  MD = ifelse(xdata.M.3_no_error$min_dist=="0.02",1,2),
  D = ifelse(xdata.M.3_no_error$Demography=="0_Simple",1,2),
  R = ifelse(xdata.M.3_no_error$Recomb.rate=="0_constant",1,2),
  GF = ifelse(xdata.M.3_no_error$GF=="0_Pulse",1,2)
)
...
Effect_size <- ulam(
  alist(
    ...
    a ~ dnorm( 0 , 0.2 ),
    c(bA,bm,bD,bR,bG) ~ dnorm( 0 , 0.5 ),
    sigma ~ dexp(1)
  ), data=Bdata, chains=4 , cores=4 )

traceplot(Effect_size)
precis(Effect_size,depth = 2)
plot(precis(Effect_size,depth = 2))
