...
Detecting admixture, i.e. gene flow between previously isolated populations can shed light into the complex evolutionary history of populations. Admixture, sometimes also referred to as hybridization, is an important factor of speciation and occurs in almost all proposed processes of it [@abbott_hybridization_2013]. In its most basic understanding, the capacity to admix is used as a criterion to characterize species [@mayr_animal_1963]. In the biological species concept species are defined as reproductively isolated from others. 
...
*bp,li 28/02: ausbauen da wichtigste assumption*
...
So the mean time estimate is known for these admixtures, but preferentially one would like to obtain the duration. especially the end of gene flow between modern humans and archaic hominin populations is of great interest. The question is how far apart is the mean time from the end of admixture.
...
Both approaches, the direct measure of the segment length and the indirect ALD based method, without first directly inferring introgressed segments such as the popular ALDER method [@loh_inferring_2013], rely on some fairly strong modelling assumptions [@pool_inference_2009; @gravel_population_2012; @liang_lengths_2014]. These assumptions are known to influence the mean time estimates and are therefore likely  influence the duration estimate as well. Therefore, we want to investigate which of these assumptions can be relaxed without causing a strong bias in the estimate.
...
is used as a clock, misspecification of the recombination map will lead to over/or 
...
ascertainment of SNPs for admixture informative sites is deployed, whereby positions in the admixed population are considered where both source populations are diverged from each other. This is usually done for archaic 
admixture to enrich the signal of ALD to not conflate it with background LD from demographic processes which is in the same range length [@sankararaman_date_2012; @fu_genome_2014]. 
