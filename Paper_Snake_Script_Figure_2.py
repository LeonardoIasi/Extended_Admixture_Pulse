...
#indivname:\t{0}/Simulation-{1}-Chr0-run{2}-{3}.ind
...
Set_name='Fig_2_D_Complex_corrected'

# Choose number of replicates #
replicates=100

# Choose Folder #
Folder_name='../Fig_2_D_Complex_corrected'

# Choose Result Folder Name #
Result_Folder='../Fig_2_D_Complex_corrected/Result'
############################################################################################
...
rule basic_simulation:
	input:
	output:
		temp("{Folder_name}/Simulation-{master_name}-Chr{nChr}-run{number}-{GF_Model}-ascertainment-0.eigenstratgeno"),
		temp("{Folder_name}/Simulation-{master_name}-Chr{nChr}-run{number}-{GF_Model}-ascertainment-0.snp"),
		temp("{Folder_name}/Simulation-{master_name}-Chr{nChr}-run{number}-{GF_Model}.ind"),
		#"{Folder_name}/Simulation-{master_name}.ind",
		temp("{Folder_name}/Simulation-{master_name}-Chr{nChr}-run{number}-{GF_Model}-ascertainment-1.eigenstratgeno"),
		temp("{Folder_name}/Simulation-{master_name}-Chr{nChr}-run{number}-{GF_Model}-ascertainment-1.snp")
		#"{Folder_name}/Simulation-{master_name}-Chr{nChr}-run{number}-{GF_Model}-ascertainment-0.eigenstratgeno",
		#"{Folder_name}/Simulation-{master_name}-Chr{nChr}-run{number}-{GF_Model}-ascertainment-0.snp",
		#"{Folder_name}/Simulation-{master_name}-Chr{nChr}-run{number}-{GF_Model}.ind",
		#"{Folder_name}/Simulation-{master_name}-Chr{nChr}-run{number}-{GF_Model}-ascertainment-1.eigenstratgeno",
		#"{Folder_name}/Simulation-{master_name}-Chr{nChr}-run{number}-{GF_Model}-ascertainment-1.snp"
	...
	run:
		...
		with open('{}'.format(output[0]),'w') as outgeno0, open('{}'.format(output[3]),'w') as outgeno1, open('{}'.format(output[1]),'w') as snpfile0, open('{}'.format(output[4]),'w') as snpfile1:
...
def merge_geno_files(wildcards):
	files = expand("{Folder_name}/Simulation-{master_name}-Chr{nChr}-run{number}-{GF_Model}-ascertainment-{Ascertainment}.eigenstratgeno",master_name=wildcards.master_name,number=wildcards.number,Set_name=Set_name,GF_Model=config['MASTER']['%s' % master_Scenario[0]]['GF_Model'][0],Folder_name=Folder_name,nChr=range(config['sim_parameters']['_MS_default_']['num_chr']),Ascertainment=wildcards.Ascertainment)
	return files

def get_ind_file(wildcards):
	files = expand("{Folder_name}/Simulation-{master_name}-Chr0-run0-{GF_Model}.ind",master_name=config['CONTROL']['%s' % Set_name],GF_Model=config['MASTER']['%s' % master_Scenario[0]]['GF_Model'][0],Folder_name=Folder_name)
	return files

rule get_ind_file:
	input:
	#	expand("{Folder_name}/Simulation-{master_name}-Chr0-run0-{GF_Model}.ind",master_name=config['CONTROL']['%s' % Set_name],GF_Model=config['MASTER']['%s' % master_Scenario[0]]['GF_Model'][0],Folder_name=Folder_name)
		#"{Folder_name}/Simulation-{master_name}-Chr0-run0-GF_Model_IV.ind"
		get_ind_file
	output:
		"{Folder_name}/Simulation-{master_name}.ind"
	run:
...
rule run_ALDER:
	input:
		...
		#"{Folder_name}/Simulation-{master_name}-Chr0-run{number}-{GF_Model}.ind"
		"{Folder_name}/Simulation-{master_name}.ind"
...
number=range(replicates),Set_name=Set_name,Folder_name=Folder_name,GF_Model=config['MASTER']['%s' % master_Scenario[0]]['GF_Model'][0],min_dist_Fit=config['MASTER']['%s' % master_Scenario[0]]['min_dist_Fit'],Ascertainment=wildcards.Ascertainment)
	return files
