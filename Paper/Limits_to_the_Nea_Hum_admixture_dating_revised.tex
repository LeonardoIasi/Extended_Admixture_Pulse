...
\hypersetup{unicode=true,
            ...
            pdfauthor={Leonardo Nicola Martin Iasi (Max Planck Institute for Evolutionary Anthropology, MPI EVA), Dr.~Harald  Ringbaier (MPI EVA),Dr.~Benjamin Marco Peter (MPI EVA, benjamin\_peter@eva.mpg.de)},
            pdfborder={0 0 0},
            breaklinks=true}
...
&=\frac{1}{\alpha \mathbb{E}[m(t)]}\int_{0}^{\infty} t^2 m(t) e^{-trl}dt
    \label{eq:standard_likelihood_definintion}
\end{align}
	
	
and we can think of $L$ as an exponential mixture distribution with mixture density $T$ \citep{ralph_geography_2013, ni_length_2016, zhou_modeling_2017}.
...
where $\Delta_x, \Delta_y$ are the difference in allele frequencies between markers in the admixing populations, and we assume that migration occurs over a short period, so that changes in the allele frequencies in the admixing populations can be neglected, and that terms of the order of $m^2$ can be neglected. At a later generation $t$ the expected LD between two markers a distance $r$ apart is
\begin{equation}
...
\label{eq:ld_general_inverted}
    D(r) = \Delta_x\Delta_y\int_0^{\infty} m(s)\exp(-rs) ds \text{.} 
\end{equation}
...
Both for the ALD and segments we use simplistic models that ignore the effects of genetic drift and recombination between introgressed segments. In the appendix section \textbf{\nameref{Appendix_1}}, we discuss these approximations and show that their impact is small in the parameter range of interest.

We note that the distribution of admixture segment lengths (Eq.! \ref{eq:standard_likelihood_definintion}) is proportional to the second derivative of the ALD function (Eq.~ \ref{eq:ld_general_inverted}) with respect to $l$. More generally, both functions uniquely determine each other (see Supp.~X). Consequently they contain, in principle, identical information to estimate admixture dates.
...
Under the simple pulse model, we assume that all fragments enter the population at the same time $t_m$. Therefore all $T_i$ have the same value $t_m$, and $T$ is a constant distribution. We can formalize this by using a Dirac delta function which integrates to one if the integration interval includes $t_m$, and zero otherwise.:
...
\begin{subequations}
The denominator of equation \ref{eq:reweighting} can be evaluated using the expectation of the Gamma distribution and thus $t_m$, and we have
...
\end{align}
\end{subequations}
for $t \geq 0$ and $k \geq 2$, which is is the density of a $\Gamma(k+1, \frac{t_m}{k})$-distribution with expectation and variance
...
\label{eq:Likelihood_function_extended_pulse}
    P(L=l | k, t_m) &= \int_{0}^{\infty} \frac{1}{\Gamma(k)(\frac{t_m}{k})^k}t^{k-1}e^{-t\frac{k}{t_m}}t e^{-tl r}dt \nonumber\\
    &= t_{m}^{-k} \ \Bigg( \frac{k+1}{l r+\frac{k}{t_{m}}}\Bigg)^{k+2}
	\text{.}
\end{align}	
...
\begin{equation}
\label{eq:Var_l_extended_pulse}
	Var[L] = \frac{(k+1)}{(k-1)} \frac{1}{(t_m r)^2}\text{.}
\end{equation}
...
The single pulse model can be thought of as the extreme case of the extended pulse model when $k \to \infty$, i.e. the pulse gets infinitely short. In the  other extreme the extended pulse model approaches a model of constant migration, i.e. the last migration event at a particular location is exponentially distributed with rate $m$. This is a model considered by \cite{pool_inference_2009}; conceptually, where we can think of each lineage in the population to enter at a rate $m$. 
...
\begin{align}
	...
	D(r) &= \Delta_X\Delta_Y \frac{m}{m + r}
\end{align}
\end{subequations}
...
We fitted the distribution to the data with a non-linear least-square optimization algorithm using the \texttt{nls} function implemented in the R \texttt{stats v.3.6.2} 
...
In Figure \ref{fig:figResult2}C we compare the fit to the data for both models and the ``true'' model. Under a constant recombination rate the true and fitted model agree. When using the variable HapMap the true and fitted models disagree for inferred segments and ALD data. Both the simple and extended pulse try to fit the uncertainties in the recombination distances, since we estimate the compound parameter $t_m r$ and only with perfectly known recombination distances $r$ equals one.
...
We contrast the effect of extended gene flow on admixture time inference with the effects of a simple/complex demographic history (Supplement Figure \ref{fig:figS1}) and a variable recombination map (\emph{i.e.} using an empirical map for simulations but assuming a constant rate for the analysis), as well as the impact of the ascertainment scheme used to amplify Neandertal introgressed variants in the analysis, the minimum genetic distance between variants ,$d_0$, in the ALD estimation and the accuracy of the ALD estimates using a simple and complex setting for each of these parameters. 


In Figure \ref{fig:figGLM}, we present the admixture time model effect sizes for these six variables and four interactions. We report the effect sizes on the standardized difference between the true and estimated mean admixture time as well as on the standardized absolute differences. The first can inform us if a variable causes a systematic bias (Supplement Table \ref{tab:table_Supplements_ests_bias}), whereas the latter is informative about a variable increasing uncertainty in our estimates (Supplement Table \ref{tab:table_Supplements_ests_deviation}). These effect sizes are estimated using a GLM on simulations under all possible parameter combinations on a scenario with admixture happening 1,500 generations ago. (Supplement Figure \ref{fig:figSGLM_data_SP} and \ref{fig:figSGLM_data_EP}).

As a baseline, for comparison, we define a standard model as one using the lower-enrichment ascertainment scheme (LES), $d_{0}$ = 0.05 cM, all SNPs used for ALD estimation, simple demography (Supplement Figure \ref{fig:figS1}A) and constant recombination rate over the chromosome. This baseline model results in unbiased estimate under the single pulse model with low deviation  of 0.08 (0.02 -- 0.14 $C.I._{95\%}$), and a slight upward bias 0.21 (0.15 -- 0.28) and slightly more variation in the estimates for the extended pulse model (0.15, 0.09 -- 0.20).

The effect of an extended period of gene flow is one of the weakest effects with -0.17 (-0.21 -0.13) for the simple pulse and also a minor effect of -0.11 (-0.15  0.07) for the extended pulse model. For both models uncertainty in the genetic distances causes a substantial overall downward bias (-1.22, -1.29 -- -1.16 SP; -0.74, -0.80 -- -0.67 EP), as well as highly increased variance in the estimates (1.26, 1.19 -- 1.32 SP; 0.82, 0.76 -- 0.88 EP). Including short-range LD causes and upward bias which is more pronounced in the extended pulse model ( 0.15, 0.08 -- 0.21 SP; 0.38, 0.31 -- 0.66 EP), with no or minor  differences in deviation. A more complex demography downward biases the estimates for both models (-0.27, -0.33 -- 0.22 SP; -0.43, -0.49 -- -0.38 EP) with some minor increases in estimate variance (0.25, -0.20 -- 0.31 SP; 0.17, 0.11 -- 0.23 EP). 


Overall the bias and deviations introduced by the ascertainment, number of SNPs and admixture model are small: around +/-0.17 standard deviations or smaller. The effect of short range LD can be easily avoided by using a longer genetic distance cutoff. The slight downward bias in the estimates from Figure \ref{fig:figResult2} can be explained by the effect of changing population sizes in the demographic model. We conclude that major uncertainties in the admixture time estimate arise from assuming a constant recombination rate. The admixture time estimates using a simple or extended admixture pulse are similarly affected by the other modeling parameters.
...
\caption{\label{fig:figGLM} Comparison of the GLM effect size estimates and 95\% C.I. for the parameters between the simple and extended pulse model: gene flow (simple/extended), recombination rate (constant/varying), demography (simple/complex), minimal genetic distance (0.02/0.05 cM), SNPs used for ALD calculation (100 \% / 5 \%) and ascertainment scheme (LES/HES). A) Effect sizes for the bias between simulated and estimated mean admixture time. B) Effect sizes for the deviation between simulated and estimated mean admixture time. Estimates are calculated across all possible combinations of parameters and are given as the estimate of the standard model plus the respective parameter estimate. Dotted horizontal line indicates unbiased admixture estimates.}
\end{figure}
...
However, we have also shown in Figure  \ref{fig:fig1_1} that inference is easier for more recent gene flow, a case that is relevant for many study systems. We investigate this in a series of simulations where we time between sampling and gene flow is smaller (Figure \ref{fig:Closer_sampling}). We use the simple demographic scenario with a constant-sized populations, and use ALD for inference using the optimised settings for the Neandertal case (ascertainment scheme = LES and  d0 = 0.05 cM).
...
In this paper, we introduce a new population genetic model for dating extended pulses of gene flow. One appealing feature of our model is that it has just two parameters, that can be interpreted as the mean time and duration of gene flow; which leads to simple closed form solutions. We show that both an instantaneous pulse and constant migration models are special cases of our model, where the duration is extremely short or long, respectively. This makes this model generally applicable beyond gene flow between Neandertals and humans, which was the motivation and focus of this study. In fact, we find that we have little resolution for the parameter settings relevant for archaic gene flow, as the data resulting from simple and extended pulses long in the past is very similar. In contrast, we have much more power to estimate the duration of gene flow from events in the recent past, a scenario is highly relevant for many systems of hybridizing species. One restriction is that we assume that the overall amount of introduced material is low, and that we ignore the effects of genetic drift and selection.
...
To test the power to distinguish the simple from an extended pulse we simulated 100, 1000, 10000 and 100000 unique times $T_i$ form a Gamma distribution implemented in the R \texttt{stats v.3.6.2} package \citep{R_Core_Team_2019}, with shape parameter $k+1$ and scale  $k/t_m$, setting $t_m$ to 1500 generations ago.  Segment lengths $L_i$ are obtained by sampling for each $T_i$ from an exponential distribution (\texttt{stats v.3.6.2} package) with rate parameter $T_i$ for present day samples and $T_i^{(closer)}= T_i - t_m - t_d/2 - 50$ for sampling 50 generations after the end of gene flow.
We fit the simple (\ref{eq:Likelihood_function_simple_pulse}) and extended pulse (\ref{eq:Likelihood_function_extended_pulse}) using the \texttt{optim} function implemented in the R \texttt{stats v.3.6.2} package.
...
number of Yoruba (YRI) and Central Europeans from Utah (CEU). For inference based on segments we simulated 50 diploid diploid non-Africans.
...
For inference, each segment is either assigned a lengths based on its physical length (``constant''), the African-American map or HapMap recombination map, depending on the inference scenario.
...
This follows from 
\begin{align}
   D(t) &= \Delta_x\Delta_y\int_{-\infty}^t m(s)\exp\left[-r(t-s) - \int_{s}^tm(x)dx\right] ds \nonumber \\
   \frac{d D}{dt} &= \frac{d}{dt}\left[ \Delta_x\Delta_y\int_{-\infty}^t m(s)\exp\left(-r(t-s)-\int_{s}^tm(x)dx\right) ds \right]\nonumber\\
   &= \Delta_x\Delta_y m(t) + \int_{-\infty}^t \Delta_x\Delta_y m(s)\left[\frac{d}{dt}\exp\left(-r(t-s)\right)\exp\left(-\int_{s}^tm(x)dx\right)\right]ds \nonumber\\
&= \Delta_x\Delta_y m(t) - (r + m(t)) \int_{-\infty}^t \Delta_x\Delta_y m(s)\exp\left(-r(t-s)\right)\exp\left(-\int_{s}^tm(x)dx\right)ds \nonumber\\   
&= \Delta_x\Delta_y m(t) - (r + m(t)) D
  \end{align}
...
\begin{align}
    D(l) = \Delta_X\Delta_Y\int_0^\infty \exp[-lrt] \exp[-M_b(t)] m_b(t) dt
\end{align}
