...
As we show in Appendix \ref{Appendix_1}), equation \ref{eq:ld_general_bwd} satisfies the differential equation
\begin{align}
...
The distribution of admixture segment lengths (equation \ref{eq:standard_likelihood_definintion}) and the ALD function (equation \ref{eq:ld_general_inverted}) are closely related -- in Appendix \ref{app:connection} we show that 
...
Equation \ref{eq:segments_continuous} differs slightly from eq. 6 in \cite{pool_inference_2009} (because we approximate the expected number of tracts with $n=Kt$, versus theirs $n=1+Kt$) but converge to each other for large $Kt$. 
...
\subsection{Comparing Population Genetic Models}\label{Model comparison}
...
\begin{itemize}
    ...
    \item Software -- LNMI
    \item Methodology -- lead: BMP -- support: HR
...
Here we describe how the admixture segment length distribution $P(l)$ and the ALD function $D(l)$ are interconnected (under the assumptions of this work) and in fact uniquely determine each other as claimed in the main text (Eq.~ and Eq.~).
...
Without loss of generality, we assume that derived allele frequencies are 0 in the target and 1 in the admixture source population, otherwise the resulting ALD curve can be re-weighted with a constant factor $A$ as in Eq.~X.
