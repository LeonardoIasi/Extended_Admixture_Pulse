...
\hypersetup{unicode=true,
            ...
            pdfauthor={Leonardo Nicola Martin Iasi (Max Planck Institute for Evolutionary Anthropology, MPI EVA), Dr.~Harald  Ringbauer (MPI EVA),
...
Thus, $D$ can be interpreted as the tail function of an exponential mixture with mixture density $m$. Alternatively, the integral in equation \ref{eq:ald_general} is also the (scaled) moment-generating function of $m$ with argument $-l$. 
...
In the easiest case, we assume that segments are perfectly known and simulate directly under the model (Eq. \ref{eq:Likelihood_function_extended_pulse}).
...
\caption{\label{fig:fig1_1} Log-likelihood ratio of the simple and extended pulse under the model. Segments are either sampled at the present (purple) or 50 generations after the end of gene flow (turquoise). Log-likelihood ratios bigger than ten are rounded to ten.}
\end{figure}
...
As the bias for ALD is lower for these old admixture events, our next goal is to more carefully evaluate the relative importance of common assumptions made in the inference of admixture times, under both the simple and extended pulse model in the ALD framework on both the bias and accuracy of estimates of $t_m$ under either model.

In particular, we use a Generalized Linear Model (GLM) framework to contrast the effect of extended gene flow on admixture time inference with i) the effects of a simple/complex demographic history (Supplementary Figure \ref{fig:figS1}), ii) a variable recombination map (\emph{i.e.} using an empirical map for simulations but assuming a constant rate for the analysis), iii) the the ascertainment scheme used to amplify Neandertal introgressed variants, iv) the minimum genetic distance between variants ,$d_0$, and v) the number of makers used to estimate the ALD curve. For each of these five modelling parameters and the two gene flow model, we have a simple model as the base case, and we study the impact of a more complex or alternative model.
...
In Figure \ref{fig:Closer_sampling}A and B, we show the accuracy of estimating $t_d$ and $t_m$ for increasingly longer pulses, samples 50 generations after gene flow ended. The corresponding comparison of model fit is depicted in Supplementary Figure \ref{fig:Closer_sampling_supplement} upper panel.
...
generations of gene flow. D) Duration estimate of the same scenario. All scenarios are simulated either under a constant recombination rate or an empirical recombination map (HapMap). Genetic distances for simulations under an empirical map are assigned by: using the same map (HapMap/HapMap), using a different map (HapMap/AAMap) and assuming a constant rate (HapMap/Constant). All times are given in generations.}
\end{figure}
...
To verify the second functional relationship (Eq.~\ref{func_rel2}) we derive $D(l)$ two times and multiply with $\frac{1}{A}\frac{G}{\mathbb{E}[K]}$:
