...
Figure \ref{fig:Closer_sampling}A and C show the mean
time estimates received from the simple and extended pulse model fit. Figure \ref{fig:Closer_sampling}B and D show the corresponding admixture duration estimate. The mean time estimates are more robust for different durations and different sampling times after the end of the admixture event using the extended pulse model under all recombination scenarios compared to the simple pulse. A slight bias is detected for sampling times very close to $t_m$ when recombination distances are uncertain. The Simple pulse displays the downward bias caused by long gene flow and admixture times are biased with uncertainties in the recombination rates. 

%Under constant recombination settings, we find that parameters under the extended pulse are well-estimated in all scenarios, although we observe a small overestimate of both $t_m$ and $t_d$, for very long admixture durations over 1000 generations in the very distant past (more than 600 generations ago). In contrast, inference under the simple pulse model results in a slight underestimate (Figure \ref{fig:Closer_sampling}, \ref{fig:Closer_sampling}A).

%We also find that the simple pulse model is fairly robust to changes in recombination settings, both using the ``correct'' and an empirically similar recombination map. Only when we do not correct for recombination rate variation, we observe a substantial underestimate by almost a factor of two for more distant admixture (Figure \ref{fig:Closer_sampling}A \& C, left panels). 

%In contrast, estimates under the extended pulse model are substantially more spread out under variable recombination maps, but we do not observe the systematic bias towards more recent admixture times as in the simple-pulse model. In contrast, the uncorrected scenario results in extremely poor estimates with very high variation, and is clearly unsuitable for this purpose (Figure \ref{fig:Closer_sampling}A \& C, right panels)

In Figure \ref{fig:Closer_sampling}C we find that sampling closer to the end of the admixture event yields unbiased estimates for $t_d$ with gene flow longer then 800 generations. Using inaccurate recombination distances Figure \ref{fig:Closer_sampling}D demonstrates that accuracy to estimate $t_d$ decreases with increases distance to the admixture event.
...
Likewise, they showed that using the SMC' model \citep{marjoram} this is
$$
