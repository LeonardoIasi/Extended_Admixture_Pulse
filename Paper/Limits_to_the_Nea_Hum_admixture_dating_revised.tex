...
\end{equation}
due to the decay of LD \cite[e.g.][]{sankararaman_date_2012}. If the  migration rate $m_f$ is a function of time, we can add up the LD introduced at each time $t$ in the past and approximate $D$ as 
\begin{equation}
...
ancestry in Africans.  For a more complex scenario of human population history, we followed \cite{skov_detecting_2018} and used a similar demographic model, but only simulated the Europeans.  We changed the $Ne$ for the ancestral humans, out-of-Africa bottleneck and ancestral Eurasians to 7,000, 250 and 5,000, respectively. The effective population sizes for Neandertals was set to 5,000 and the split time of non-Africans are kept the same as in the ALD simulations (2550 generations ago) (Supplementary Figure 1B). 
...
\begin{align*}
\frac{\mathbb{E}[K]}{G} \int_l^\infty P(x) (x-l) dx &= \int_l^\infty \diff x \int_{0}^{\infty} \diff t\ t^2 m(t) \exp(-xt) (x-l)  \\
...
%\input{Limits_to_the_Nea_Hum_admixture_dating_revised.bbl}
