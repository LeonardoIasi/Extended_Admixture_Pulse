...
As non-native English speaker, we were not aware of the linguistic subtlety between ``limits'' and  ``limitations'', and suspect that many readers also will not be too concerned about this. We changed the title to ``An extended admixture pulse model reveals the limitations to Human-Neandertal introgression dating''. The latter change was necessary to remain under the 100-character title limit.
...
We do report all major previous (genetic) estimates of Neandertal gene flow dates in the introduction (section Neandertal gene flow estimates). We now also briefly discuss our results, and the inability to distinguish various events in the results section ``Application to Neandertal data''.  \textbf{TODO add Discussion section}.
...
\textbf{TODO}
Add to discussion: Theory is general, and not restricted by model system, but assumes admixture fraction is low. For simulations, we focus on scenario of Neandertal-Human admixture because this was our primary goal. Other events at similar time-scales should behave similarly (i.e. Denisovan admixture, or other events on the order of thousands of generations). 
...
We agree that the biases in the estimates of $t_d$ and $t_m$ are unfortunate, and we note that the entire section ``Comparing effect sizes for technical covariates'' deals with investigating the various sources of bias of $t_m$. 

We improved this in the current version of the manuscript as follows:

\emph{Theory section} \textbf{TODO} We added a section where we discuss some model assumptions we make for simplification, and discuss how they impact our inference.
\emph{Analysis}: 
\emph{Discussion:}
...
We verified that we are outside the parameter range that this is a concern by redoing the most extreme scenario we use using either simulation scheme. We do not find any difference between the two models (see attached figure), so we feel confident that this likely will not impact our results.
...
We do agree that these elaborations are intuitively reasonable, but to show where/if they hold we would have to develop an SMC or SMC'-like model in quite some detail, which we feel is beyond the scope of the current paper. 

Moreover, we found that with the way that we consider linkage disequilibrium separately, we are able to include the migration fraction in a much more direct way in our model.

make sense under the assumption that recombination during the admixture event can be neglected. However, we also found (based on comments on our preprint) that our derivation of the ALD was slightly off, so we decided to derive it from scratch, and it turns out that the admixture fraction is included as a constant in the ALD, so this should not lead to a bias. Genetic drift is another concern, and we discuss in the new theory supplemental section \textbf{Approximation details}.

\textbf{Data}: Add (1-m)-term is not sufficient, likely due to drift.
...
In \textbf{Figure 3C}, we now show the fitted curves for both true segments, inferred segments as well as the raw ALD data. We think this shows that the likelihood inference procedure or ALD fitting is unlikely to be a source of major bias.
...
We address these concern in \textbf{figure 4} and section Compariong effect sizes for covariates. 

\textbf{TODO}: Model with variance/deviation
...
We found numerous papers that based their interpretations (partially) on this, based on this work, flawed interpretation (see below).
...
\textbf{To make this point more clear, we rephrased this section as follows:}


Previous estimates to date Neandertal-human gene flow have focused almost entirely on the mean time of gene flow using a simple pulse model, for which reasonably tight credible intervals can be estimated. Under this model, the credible intervals of this time are bounds of when gene flow between Neandertals and early modern humans could have happened. Here, we show that the observed data is compatible with multiple scenarios, and that models involving hundreds of generations of gene flow are indistinguishable from shorter gene flow events. This means that while substantial amounts of gene flow happened around these mean times, gene flow might have also happened tens of thousands of years before or after. This is of great practical importance, as it makes linking genetic admixture date estimates with biogeographical events much more difficult
\end{document}